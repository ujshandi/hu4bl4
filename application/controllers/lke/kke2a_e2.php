...
class Kke2a_e2 extends CI_Controller {
	var $objectId = 'kke2a_e2';
	function __construct()
	{
		...
		$this->load->model('/lke/kke2a_e2_model');
	...
	function index(){
		...
		$data['renstra_ip_radio'] = $this->lke_konversi_model->getListIndex($this->objectId,array('jenis_lke'=>'kke2a','unit_kerja'=>'e2'),true,"renstra_ip");
		$data['rkt_ip_radio'] = $this->lke_konversi_model->getListIndex($this->objectId,array('jenis_lke'=>'kke2a','unit_kerja'=>'e2'),true,"rkt_ip");
		$data['pk_ip_radio'] = $this->lke_konversi_model->getListIndex($this->objectId,array('jenis_lke'=>'kke2a','unit_kerja'=>'e2'),true,"pk_ip");
		
		$data['listIndex_renstra_ip'] = $this->lke_konversi_model->getCountIndex(array('jenis_lke'=>'kke2a','unit_kerja'=>'e2'),true);
		$data['listIndex_rkt_ip'] = $this->lke_konversi_model->getCountIndex(array('jenis_lke'=>'kke2a','unit_kerja'=>'e2'),true);
		$data['listIndex_pk_ip'] = $this->lke_konversi_model->getCountIndex(array('jenis_lke'=>'kke2a','unit_kerja'=>'e2'),true);
	  	$this->load->view('lke/kke2a_e2_v',$data);
	}
	
	
	private function get_form_values() {
		$dt['tahun'] = $this->input->post("tahun", TRUE); 
		$dt['kke2a_e2_id'] = $this->input->post("kke2a_e2_id", TRUE); 
		...
		$dt['renstra_ip_nilai'] = $this->lke_konversi_model->getKonversi('kke2a',$dt['renstra_ip'],'e2');
		$dt['rkt_ip'] = $this->input->post("rkt_ip", TRUE); 
		$dt['rkt_ip_nilai'] = $this->lke_konversi_model->getKonversi('kke2a',$dt['rkt_ip'],'e2');
		$dt['pk_ip'] = $this->input->post("pk_ip", TRUE); 
		$dt['pk_ip_nilai'] = $this->lke_konversi_model->getKonversi('kke2a',$dt['pk_ip'],'e2');
	...
	function save(){
		...
		$this->form_validation->set_rules("kode_e2", 'Unit Kerja Eselon I', 'trim|required|xss_clean');
	//	$this->form_validation->set_rules("index_mutu", 'Index Mutu', 'trim|required|xss_clean');
		...
		if ($this->form_validation->run() == FALSE){ // jika tidak valid
			...
			// validasi detail
				...
				if ($data['kke2a_e2_id']==''){	
					$result = $this->kke2a_e2_model->InsertOnDb($data,$data['pesan_error']);
				}
				else {
					$result = $this->kke2a_e2_model->UpdateOnDb($data,$data['kke2a_e2_id']);
				}
	...
	function grid($filtahun=null,$file2=null,$filsasaran=null,$filiku=null){
		...
		echo $this->kke2a_e2_model->easyGrid($filtahun,$file2,$filsasaran,$filiku);
	}
	
	public function excel($filtahun=null,$file2=null,$filsasaran=null,$filiku=null,$page=null,$rows=null){
		echo  $this->kke2a_e2_model->easyGrid($filtahun,$file2,$filsasaran,$filiku,3,$page,$rows);
	}
	...
	public function pdf($filtahun=null,$file2=null,$filsasaran=null,$filiku=null,$page=null,$rows=null){
		...
		$pdfdata = $this->kke2a_e2_model->easyGrid($filtahun,$file2,$filsasaran,$filiku,2,$page,$rows);
	...
	function getSatuan($id){
		echo $this->kke2a_e2_model->getSatuan($id);
	}
