...
class Ongoing_e1 extends CI_Controller {
	
	var $objectId = 'ongoinge1';
	...
	function __construct()
	{
		...
		$this->load->model('/kka/ongoing_e1_model');
	...
	function index(){
		...
		//$data['formLookupTarif'] = $this->tarif_model->lookup('#winLookTarif'.$data['objectId'],"#medrek_id".$data['objectId']);
	  	$this->load->view('kka/ongoing_e1s_v',$data);
	}
	
	public function add(){
		$data['title'] = 'Add Data Ongoing Monev Eselon I Pagu Usulan';	
		$data['objectId'] = $this->objectId;
		//$data['formLookupTarif'] = $this->tarif_model->lookup('#winLookTarif'.$data['objectId'],"#medrek_id".$data['objectId']);
	...
	public function edit($id, $editmode='true'){
		...
		$data['title'] = ($editmode==TRUE?'Edit':'View').' Data Ongoing Monev Eselon I Pagu Usulan';
		$data['objectId'] = $this->objectId;
		
		$data['result'] = $this->ongoing_e1_model->getDataEdit($id);
	...
	function grid($filtahun=null,$file1=null){
		...
		echo $this->ongoing_e1_model->easyGrid($filtahun,$file1);
	}
	
	function gridMonev($filtahun=null,$kodekegiatan=null){
		
		echo $this->ongoing_e1_model->easySubGrid($filtahun,$kodekegiatan);
	}
	...
	function gridKegiatan($filtahun=null,$file1=null,$file2=null){
		...
		echo $this->kegiatankl_model->easyGrid($file1,$file2,$filtahun,1,'ongoing');
	}
		...
		if ($this->form_validation->run() == FALSE){ // jika tidak valid
			...
			if ($data['kode_sasaran_e1']=='0') {
				...
				if($this->check_detail($data, $pesan)){
					$result = $this->ongoing_e1_model->InsertOnDb($data);
				}else{
	...
	function check_detail($data, & $pesan){
		...
		/* 	// cek ke database
			if($this->ongoing_e1_model->data_exist($data['tahun'], $data['kode_e1'], $data['kode_sasaran_e1'], $r['kode_iku_e1'])){ 
	...
	function save_edit(){
		...
		if($data['target'] == '' || $data['target'] == null){
			...
			if($data['old_kode_iku_e1'] != $data['kode_iku_e1']){ // jika ada perubahan kode iku
				if(!$this->ongoing_e1_model->data_exist($data['tahun'], $data['kode_e1'], $data['kode_sasaran_e1'], $data['kode_iku_e1'])){
					$result = $this->ongoing_e1_model->UpdateOnDb($data);
				}else{
					$result = false;
	...
	function delete($id=''){
		if($id != ''){
			$result = $this->ongoing_e1_model->DeleteOnDb($id);
			if ($result){
	...
	function getSatuan($id){
		if($id != '0'){
			echo $this->ongoing_e1_model->getSatuan($id);
		}
	}
	
	function getIKU_e1($kode, $tahun=""){
		if($kode != '0' && $tahun != ""){
			echo $this->ongoing_e1_model->getIKU_e1($this->objectId, $kode, $tahun);
		}
	}
	
	function getKegiatan_e1($kode, $tahun=""){
		if($kode != '0' && $tahun != ""){
			echo $this->ongoing_e1_model->getKegiatan_e1($this->objectId, $kode, $tahun);
		}
	}
	...
	public function pdf($file1=null){
		...
		$pdfdata = $this->ongoing_e1_model->easyGrid($file1,2);
	...
	public function excel($file1=null){
		echo  $this->ongoing_e1_model->easyGrid($file1,3);
	}
}
?>