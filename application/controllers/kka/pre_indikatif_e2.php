...
class Pre_indikatif_e2 extends CI_Controller {
	
	var $objectId = 'preindikatife2';
	...
	function __construct()
	{
		...
		$this->load->model('/kka/pre_indikatif_e2_model');
	...
	function index(){
		...
		//$data['formLookupTarif'] = $this->tarif_model->lookup('#winLookTarif'.$data['objectId'],"#medrek_id".$data['objectId']);
	  	$this->load->view('kka/pre_indikatif_e2s_v',$data);
	}
	...
	public function add(){
		...
		$data['objectId'] = $this->objectId;
		//$data['formLookupTarif'] = $this->tarif_model->lookup('#winLookTarif'.$data['objectId'],"#medrek_id".$data['objectId']);
	  	$this->load->view('kka/pre_indikatif_e2_v',$data);
	}
		...
		if ($this->form_validation->run() == FALSE){ // jika tidak valid
			...
			if ($data['kode_sasaran_e2']=='0') {
				...
				if($this->check_detail($data, $pesan)){
					$result = $this->pre_indikatif_e2_model->InsertOnDb($data);
				}else{
	...
	function check_detail($data, & $pesan){
		...
		/* 	// cek ke database
			if($this->pre_indikatif_e2_model->data_exist($data['tahun'], $data['kode_e1'], $data['kode_sasaran_e1'], $r['kode_ikk'])){ 
	...
	function delete($id=''){
		if($id != ''){
			$result = $this->pre_indikatif_e2_model->DeleteOnDb($id);
			if ($result){
	...
	function getSatuan($id){
		if($id != '0'){
			echo $this->pre_indikatif_e2_model->getSatuan($id);
		}
	}
	
	function getikk($kode, $tahun=""){
		if($kode != '0' && $tahun != ""){
			echo $this->pre_indikatif_e2_model->getikk($this->objectId, $kode, $tahun);
		}
	}
	
	function getKegiatan_e2($kode, $tahun="",$kodesasaran="",$kodeikk=""){
		if($kode != '0' && $tahun != ""){
			echo $this->pre_indikatif_e2_model->getKegiatan_e2($this->objectId, $kode, $tahun,true,$kodesasaran,$kodeikk);
		}
	}
	...
	public function pdf($file1=null){
		...
		$pdfdata = $this->pre_indikatif_e2_model->easyGrid($file1,2);
	...
	public function excel($file1=null){
		echo  $this->pre_indikatif_e2_model->easyGrid($file1,3);
	}
}
?>
