...
class Ongoing_e2 extends CI_Controller {
	...
	public function edit($id, $editmode='true'){
		...
		$data['result'] = $this->ongoing_e2_model->getDataEdit($id);
	  	...
	  	$this->load->view('kka/ongoing_e2_v_edit',$data);
	}
	...
	function save_edit(){
		...
		if($data['target'] == '' || $data['target'] == null){
			...
			if($data['old_kode_ikk'] != $data['kode_ikk']){ // jika ada perubahan kode iku
				if(!$this->ongoing_e2_model->data_exist($data['tahun'], $data['kode_e1'], $data['kode_sasaran_e1'], $data['kode_ikk'])){
	...
	function getKegiatan_e2($kode, $tahun=""){
		if($kode != '0' && $tahun != ""){
			echo $this->ongoing_e2_model->getKegiatan_e2($this->objectId, $kode, $tahun);
		}
	}
