	...
	*/
	...
	public function easyGrid($filtahun=null,$file2=null,$purpose=1){
		...
		if ($count>0){
			...
			$this->db->select('distinct a.ongoinge2_id, a.tahun, a.kode_ikk, a.kode_e2 as rkt_kode_e2, a.kode_sasaran_e2 AS kode_sasaran_e2, b.deskripsi, a.jumlah,k.kode_kegiatan,k.nama_kegiatan,subkl.kode_subkegiatan, subkl.nama_subkegiatan',false);
	...
	public function getKegiatan_e2($objectId, $kode, $tahun){
		...
		foreach($que->result() as $r){
			...
			$this->db->join('tbl_pre_definitif_e2 pre','s.kode_subkegiatan=pre.kode_subkegiatan and s.tahun=pre.tahun');
			...
			$out .= '<tr>
					...
					<td align="right"><input class="money" readonly id="anggaran_'.$i.'" name="detail['.$i.'][anggaran]" style="text-align:right" value="'.$r->anggaran.'" size="20" /></td>					
				...
				//	var_dump((int)$z->preusulan1_e2_id);
					$checked = ((int)$z->preindikatif_e2_id>0?"checked":"");
					$out .= '<tr>
					<td>'.($i+1).'</td>
