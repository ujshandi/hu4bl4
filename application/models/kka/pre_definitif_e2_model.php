	...
	*/
	...
	public function getKegiatan_e2($objectId, $kode, $tahun,$exclude=false,$kodesasaran="",$kodeikk=""){
		...
		foreach($que->result() as $r){
			...
			$this->db->flush_cache();
			$this->db->select('s.*,pre.preindikatif_e2_id,pre.jumlah');
			$this->db->from('tbl_subkegiatan_kl s',false);
			$this->db->join('tbl_pre_indikatif_e2 pre','s.kode_subkegiatan=pre.kode_subkegiatan and s.tahun=pre.tahun','left');
