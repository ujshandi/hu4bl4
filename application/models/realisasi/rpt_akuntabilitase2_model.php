...
class Rpt_akuntabilitase2_model extends CI_Model
{	
	/**
	* constructor
	*/
	...
	public function easyGrid($filtahun=null,$filsasaran=null,$filiku=null,$file1=null,$file2=null,$purpose=1,$pageNumber=null,$pageSize=null){
		...
		if ($count>0){
			...
			$this->db->select("sasaran.deskripsi as sasaran_strategis, count(iku.kode_ikk) as indikator_kinerja, sasaran.kode_sasaran_e2",false);
			//$this->db->from('tbl_rkt_kl');
			$this->db->from('tbl_kinerja_eselon2 rkt inner join tbl_ikk iku on iku.kode_ikk = rkt.kode_ikk
inner join tbl_sasaran_eselon2 sasaran on sasaran.kode_sasaran_e2 = rkt.kode_sasaran_e2 inner join tbl_eselon2 e2 on e2.kode_e2=rkt.kode_e2',false);
			$query = $this->db->get();
			...
			foreach ($query->result() as $row)
			{
				...
				$response->rows[$i]['pejabat']=$this->getPejabatE2($row->kode_sasaran_e2,$filtahun);
	...
	//-----------------------------------
		if ($purpose==1) //grid normal
			...
			to_excel($query,"SebaranIkkEselon2",$colHeaders);
		}
	...
	public function GetRecordCount($filtahun=null,$filsasaran=null,$filiku=null,$file1=null,$file2=null){
		...
		$sql = 'select count(*) as num_rows from (select sasaran.deskripsi from tbl_kinerja_eselon2 rkt inner join tbl_ikk iku on iku.kode_ikk = rkt.kode_ikk inner join tbl_sasaran_eselon2 sasaran on sasaran.kode_sasaran_e2 = rkt.kode_sasaran_e2 inner join tbl_eselon2 e2 on e2.kode_e2=rkt.kode_e2'.$where.' GROUP BY sasaran.deskripsi, sasaran.kode_sasaran_e2 ) as t1';
		$q = $this->db->query($sql);
	...
	public function getPejabatE2($sasaranE1,$filtahun){
		$this->db->flush_cache();
		$this->db->select('e2.singkatan , concat(count(iku.kode_ikk), \' IKU\') as pejabat',false);
		$this->db->from('tbl_kinerja_eselon2 rkt inner join tbl_ikk iku on iku.kode_ikk = rkt.kode_ikk
inner join tbl_sasaran_eselon2 sasaran on sasaran.kode_sasaran_e2 = rkt.kode_sasaran_e2
inner join tbl_eselon2 e2 on e2.kode_e2=iku.kode_e2',false);
		$this->db->group_by('e2.singkatan');
		if($filtahun != '' && $filtahun != '-1' && $filtahun != null) {
				$this->db->where("rkt.tahun",$filtahun);
			}	
		$this->db->where('sasaran.kode_sasaran_e2',$sasaranE1);
		$query = $this->db->get();
		$out="";
