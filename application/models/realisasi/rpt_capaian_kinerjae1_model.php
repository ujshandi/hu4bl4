...
class Rpt_capaian_kinerjae1_model extends CI_Model
{	
	/**
	* constructor
	*/
	...
	public function easyGrid($filtahun=null,$filsasaran=null,$filiku=null,$file1=null,$filstart=null,$filend=null,$purpose=1,$pageNumber=null,$pageSize=null){
		...
		$count = $this->GetRecordCount($filtahun,$filsasaran,$filiku,$file1,$filstart,$filend);
		...
		//	$this->db->order_by($sort." ".$order );
			$this->db->order_by("iku.kode_iku_e1");
			...
			$this->db->select("iku.deskripsi as indikator_kinerja,iku.satuan,iku.kode_iku_e1",false);
			
			$this->db->from('tbl_iku_eselon1 iku',false);
			$query = $this->db->get();
			...
			foreach ($query->result() as $row){
				...
				$row->target = $this->getTarget($filtahun,$row->kode_iku_e1);
				...
				//$row->realisasi = $row->triwulan1+$row->triwulan2+$row->triwulan3+$row->triwulan4;//$this->getRealisasi($filtahun,$row->kode_iku_e1,"-1");
		...
		*/
		if ($purpose==1) //grid normal
			...
			to_excel($query,"CapaianKinerjaE1",$colHeaders);
		}
	...
	//	 $sql = 'select count(*) as num_rows from (SELECT DISTINCT iku.deskripsi as indikator_kinerja,iku.satuan,pk.target, rkt.realisasi FROM tbl_kinerja_eselon1 rkt inner join tbl_iku_eselon1 iku on iku.kode_iku_e1 = rkt.kode_iku_e1 inner join tbl_sasaran_eselon1 sasaran on sasaran.kode_sasaran_e1 = rkt.kode_sasaran_e1 inner join tbl_pk_eselon1 pk on pk.kode_sasaran_e1 = rkt.kode_sasaran_e1 and pk.kode_iku_e1=rkt.kode_iku_e1'.$where.'  ) as t1';
		 $sql = 'select count(*) as num_rows from (SELECT DISTINCT iku.deskripsi as indikator_kinerja,iku.satuan FROM tbl_iku_eselon1 iku '.$where.'  ) as t1';
		$q = $this->db->query($sql);
