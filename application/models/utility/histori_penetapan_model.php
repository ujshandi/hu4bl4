...
class histori_penetapan_model extends CI_Model
{	
	/**
	* constructor
	*/
	...
	public function KL_grid($filtahun=null,$purpose=1){
		...
		$sort = isset($_POST['sort']) ? strval($_POST['sort']) : 'id_pk_kl';  
		...
		if ($count>0){
			...
			$this->db->order_by("a.id_pk_kl ASC");
			$query = $this->db->get();
			...
			//utk kepentingan export pdf===================
				$pdfdata[] = array($no,$response->rows[$i]['kode_sasaran_kl'],$response->rows[$i]['kode_iku_kl'],$response->rows[$i]['target'], $response->rows[$i]['penetapan'],$response->rows[$i]['log_status'], $response->rows[$i]['log_user'], $response->rows[$i]['log_date']);
			//============================================================
				$i++;
			} 
		...
		if ($purpose==1) //grid normal
			...
			$colHeaders = array("Tahun","Kode Sasaran","Kode IKU","Target","Penetapan","Log");		
			//var_dump($query->result());die;
			to_excel($query,"PKKementerian",$colHeaders);
		}
	...
	public function KL_grid_count($filtahun=null){		
		...
		$query=$this->db->from('tbl_pk_kl_log');
	...
	public function E1_grid($filtahun=null,$file1=null,$purpose=1){
		...
		if ($count>0){
			...
			$this->db->order_by($sort." ".$order );
			if($purpose==1){$this->db->limit($limit,$offset);}
			$this->db->select("a.tahun, a.kode_e1, b.nama_e1, a.kode_sasaran_e1, c.deskripsi AS deskripsi_sasaran_e1, a.kode_iku_e1, d.deskripsi AS deskripsi_iku_e1, a.target, a.penetapan, a.log", false);
			$this->db->from('tbl_pk_eselon1_log a');
			...
			$this->db->order_by("a.kode_e1 ASC, a.id_pk_e1 ASC");
			$query = $this->db->get();
			...
			foreach ($query->result() as $row)
			{
				...
				//utk kepentingan export pdf===================
				if($file1 != '' && $file1 != '-1' && $file1 != null)
					$pdfdata[] = array($no,$response->rows[$i]['kode_sasaran_e1'],$response->rows[$i]['deskripsi_sasaran_e1'],$response->rows[$i]['kode_iku_e1'],$response->rows[$i]['deskripsi_iku_e1'],$response->rows[$i]['target'], $response->rows[$i]['penetapan'], $response->rows[$i]['log_status'], $response->rows[$i]['log_user'], $response->rows[$i]['log_date']);
				else
					$pdfdata[] = array($no,$response->rows[$i]['kode_e1'],$response->rows[$i]['kode_sasaran_e1'],$response->rows[$i]['deskripsi_sasaran_e1'],$response->rows[$i]['kode_iku_e1'],$response->rows[$i]['deskripsi_iku_e1'],$response->rows[$i]['target'], $response->rows[$i]['penetapan'], $response->rows[$i]['log_status'], $response->rows[$i]['log_user'], $response->rows[$i]['log_date']);
				//============================================================
				$i++;
			} 
		...
		if ($purpose==1) //grid normal
			...
			//tambahkan header kolom
			if($file1 != '' && $file1 != '-1' && $file1 != null)
				$colHeaders = array("Tahun", "Kode Sasaran", "Kode IKU", "Target", "Penetapan");
			else
				$colHeaders = array("Tahun", "Kode Eselon I", "Kode Sasaran", "Kode IKU", "Target", "Penetapan");		
		//	var_dump($query->result());die;
			to_excel($query,"PKEselon1",$colHeaders);
		}
	}
	...
	public function E2_grid($filtahun=null,$file1=null,$file2=null,$purpose=1){
		...
		if ($count>0){
			...
			$this->db->order_by($sort." ".$order );
			if($purpose==1){$this->db->limit($limit,$offset);}
			$this->db->select("a.tahun, a.kode_e2, b.nama_e2, a.kode_sasaran_e2, c.deskripsi AS deskripsi_sasaran_e2, a.kode_ikk, d.deskripsi AS deskripsi_ikk, a.target, a.penetapan, a.log", false);
			$this->db->from('tbl_pk_eselon2_log a');
			...
			$this->db->order_by("a.kode_e2 ASC, a.id_pk_e2 ASC");
			$query = $this->db->get();
			...
			foreach ($query->result() as $row)
			{
				...
				//utk kepentingan export pdf===================
				if($file2 != '' && $file2 != '-1' && $file2 != null)
					$pdfdata[] = array($no,$response->rows[$i]['kode_sasaran_e2'],$response->rows[$i]['deskripsi_sasaran_e2'],$response->rows[$i]['kode_ikk'],$response->rows[$i]['deskripsi_ikk'],$response->rows[$i]['target'], $response->rows[$i]['penetapan'], $response->rows[$i]['log_status'], $response->rows[$i]['log_user'], $response->rows[$i]['log_date']);
				else
					$pdfdata[] = array($no,$response->rows[$i]['kode_e2'],$response->rows[$i]['kode_sasaran_e2'],$response->rows[$i]['deskripsi_sasaran_e2'],$response->rows[$i]['kode_ikk'],$response->rows[$i]['deskripsi_ikk'],$response->rows[$i]['target'], $response->rows[$i]['penetapan'], $response->rows[$i]['log_status'], $response->rows[$i]['log_user'], $response->rows[$i]['log_date']);
				//============================================================
				$i++;
			} 
		...
		if ($purpose==1) //grid normal
			...
			//tambahkan header kolom
			if($file2 != '' && $file2 != '-1' && $file2 != null)
				$colHeaders = array("Tahun", "Kode Sasaran", "Kode IKK", "Target", "Penetapan");
			else
				$colHeaders = array("Tahun", "Kode Eselon II", "Kode Sasaran", "Kode IKK", "Target", "Penetapan");		
		//	var_dump($query->result());die;
