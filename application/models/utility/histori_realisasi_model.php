...
class histori_realisasi_model extends CI_Model
{	
	/**
	* constructor
	*/
	...
	public function KL_grid($filtahun=null,$purpose=1){
		...
		$sort = isset($_POST['sort']) ? strval($_POST['sort']) : 'id_kinerja_kl';  
		...
		if ($count>0){
			...
			$this->db->from('tbl_kinerja_kl_log a');
			...
			$this->db->order_by("a.id_kinerja_kl ASC");
			$query = $this->db->get();
			...
			//utk kepentingan export pdf===================
				$pdfdata[] = array($no,$response->rows[$i]['triwulan'],$response->rows[$i]['kode_sasaran_kl'],$response->rows[$i]['kode_iku_kl'],$response->rows[$i]['realisasi'], $response->rows[$i]['log_status'], $response->rows[$i]['log_user'], $response->rows[$i]['log_date']);
			//============================================================
				$i++;
			} 
		...
		if ($purpose==1) //grid normal
			...
			$colHeaders = array("Tahun","Bulan","Kode Sasaran","Kode IKU","Realisasi","Log");		
			//var_dump($query->result());die;
			to_excel($query,"RealisasiKementerian",$colHeaders);
		}
	...
	public function KL_grid_count($filtahun=null){		
		...
		$query=$this->db->from('tbl_kinerja_kl_log');
	...
	public function E1_grid($filtahun=null,$file1=null,$purpose=1){
		...
		if ($count>0){
			...
			$this->db->order_by($sort." ".$order );
			if($purpose==1){$this->db->limit($limit,$offset);}
			$this->db->select("a.tahun, a.triwulan, a.kode_e1, b.nama_e1, a.kode_sasaran_e1, c.deskripsi AS deskripsi_sasaran_e1, a.kode_iku_e1, d.deskripsi AS deskripsi_iku_e1, a.realisasi, a.log", false);
			$this->db->from('tbl_kinerja_eselon1_log a');
			...
			$this->db->order_by("a.kode_e1 ASC, a.id_kinerja_e1 ASC");
			$query = $this->db->get();
			...
			foreach ($query->result() as $row)
			{
				...
				//utk kepentingan export pdf===================
				if($file1 != '' && $file1 != '-1' && $file1 != null)
					$pdfdata[] = array($no,$response->rows[$i]['triwulan'],$response->rows[$i]['kode_sasaran_e1'],$response->rows[$i]['deskripsi_sasaran_e1'],$response->rows[$i]['kode_iku_e1'],$response->rows[$i]['deskripsi_iku_e1'],$response->rows[$i]['realisasi'], $response->rows[$i]['log_status'], $response->rows[$i]['log_user'], $response->rows[$i]['log_date']);
				else
					$pdfdata[] = array($no,$response->rows[$i]['triwulan'],$response->rows[$i]['kode_e1'],$response->rows[$i]['kode_sasaran_e1'],$response->rows[$i]['deskripsi_sasaran_e1'],$response->rows[$i]['kode_iku_e1'],$response->rows[$i]['deskripsi_iku_e1'],$response->rows[$i]['realisasi'], $response->rows[$i]['log_status'], $response->rows[$i]['log_user'], $response->rows[$i]['log_date']);
				//============================================================
				$i++;
			} 
		...
		if ($purpose==1) //grid normal
			...
			//tambahkan header kolom
			if($file1 != '' && $file1 != '-1' && $file1 != null)
				$colHeaders = array("Tahun", "Bulan", "Kode Sasaran", "Kode IKU", "Realisasi");
			else
				$colHeaders = array("Tahun", "Bulan", "Kode Eselon I", "Kode Sasaran", "Kode IKU", "Realisasi");		
		//	var_dump($query->result());die;
			to_excel($query,"RealisasiEselon1",$colHeaders);
		}
	}
	...
	public function E2_grid($filtahun=null,$file1=null,$file2=null,$purpose=1){
		...
		if ($count>0){
			...
			$this->db->order_by($sort." ".$order );
			if($purpose==1){$this->db->limit($limit,$offset);}
			$this->db->select("a.tahun, a.triwulan, a.kode_e2, b.nama_e2, a.kode_sasaran_e2, c.deskripsi AS deskripsi_sasaran_e2, a.kode_ikk, d.deskripsi AS deskripsi_ikk, a.realisasi, a.log", false);
			$this->db->from('tbl_kinerja_eselon2_log a');
			...
			$this->db->order_by("a.kode_e2 ASC, a.id_kinerja_e2 ASC");
			$query = $this->db->get();
			...
			foreach ($query->result() as $row)
			{
				...
				//utk kepentingan export pdf===================
				if($file2 != '' && $file2 != '-1' && $file2 != null)
					$pdfdata[] = array($no,$response->rows[$i]['triwulan'],$response->rows[$i]['kode_sasaran_e2'],$response->rows[$i]['deskripsi_sasaran_e2'],$response->rows[$i]['kode_ikk'],$response->rows[$i]['deskripsi_ikk'],$response->rows[$i]['realisasi'], $response->rows[$i]['log_status'], $response->rows[$i]['log_user'], $response->rows[$i]['log_date']);
				else
					$pdfdata[] = array($no,$response->rows[$i]['triwulan'],$response->rows[$i]['kode_e2'],$response->rows[$i]['kode_sasaran_e2'],$response->rows[$i]['deskripsi_sasaran_e2'],$response->rows[$i]['kode_ikk'],$response->rows[$i]['deskripsi_ikk'],$response->rows[$i]['realisasi'], $response->rows[$i]['log_status'], $response->rows[$i]['log_user'], $response->rows[$i]['log_date']);
				//============================================================
				$i++;
			} 
		...
		if ($purpose==1) //grid normal
			...
			//tambahkan header kolom
			if($file2 != '' && $file2 != '-1' && $file2 != null)
				$colHeaders = array("Tahun", "Bulan", "Kode Sasaran", "Kode IKK", "Realisasi");
			else
				$colHeaders = array("Tahun", "Bulan", "Kode Eselon II", "Kode Sasaran", "Kode IKK", "Realisasi");		
		//	var_dump($query->result());die;
