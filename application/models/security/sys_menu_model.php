...
{    var $shorcutList;
	...
	*/
	...
	public function loadMegaMenu($app_type,$imported=null,$cekPolicy=false){
		...
		$result = "";
		foreach ($query->result() as $row){
			$result .= "<li><a href='#'>$row->menu_name</a>";
		...
		//	if ($row->menu_id == "1")
			$child = $this->loadMegaChild($row->menu_id,$app_type,$imported,$cekPolicy,$policy);
			if ($child!="")
				$result .= "<ul>".$child."</ul>";
			$result .= "</li>";
		}	
...
//				var_dump($currentPolicy."=".$row->menu_id.'='.$this->session->userdata('group_id'));
	...
	public function loadMegaChild($menuparent,$app_type,$imported=null,$cekPolicy=false,& $policy){
		$sql = 'select m.* from tbl_menu m '
		.' where hide<>1 and menu_parent = '.$menuparent
		."  and app_types like '%$app_type%' "
		.($imported==null?'  ':' and imported = '.$imported)
		...
		foreach ($query->result() as $row){
			...
			$childs = $this->loadMegaChild($row->menu_id,$app_type,$imported,$cekPolicy,$policy);
			
			if ($childs==""){ 
				//if($i==0)
				//	$result .= "<ul>";
				$result .= '<li><a href="#" onclick="addTab(\''.$row->menu_name.'\',\''.$row->url.'\')">'.$row->menu_name.'</a></li>';
			}
			else {
				$result .= "<li><a href='#'>$row->menu_name</a>";
				
				$result .= "<ul>".$childs."</ul></li>";
	...
	function prepareMenu($group_id,$parentId,$result=""){	    
		...
		foreach ($menu as $i=>$row){
			...
			//echo '<ul>';
			$rs = $this->prepareMenu($group_id,$menu_id,$rs);
