		...
		function setListE2<?=$objectId?>(){
			$("#divEselon2<?=$objectId?>").load(
				...
				function (){
					...
					setSasaran<?=$objectId?>($("#kode_e2<?=$objectId?>").val(), $('#tahun<?=$objectId;?>').val());
				}
			);
		 }
...
<table id="dg<?=$objectId;?>" class="easyui-datagrid" style="height:auto;width:auto" title="Data Capaian IKU Eselon II"  fitColumns="true" singleSelect="true" nowrap="false" rownumbers="true" pagination="true" showFooter="true">
	  <thead>
	  <tr>
...
$(document).ready(function(){
	...
	getListSasaran<?=$objectId?> = function (tahun,e2){
			<? if ($this->session->userdata('unit_kerja_e2')!='-1') {?>
				 e2 = '<?=$this->session->userdata('unit_kerja_e2');?>';
				 $("#filter_e2<?=$objectId;?>").val(e2);
				<?}?>
			...
			if ((tahun==null)||(tahun=="")) tahun = "-1";
				$("#divSasaranKL<?=$objectId?>").load(
					base_url+"pengaturan/sasaran_eselon2/getListSasaranE2/"+"<?=$objectId;?>"+"/"+e2+"/"+tahun,
					function(){
						$('textarea').autosize();   
	...
	getListSasaran<?=$objectId;?>($("#filter_tahun<?=$objectId;?>").val(),$("#filter_e2<?=$objectId;?>").val());
	
	setSasaran<?=$objectId?> = function(kode){
		//do nothing
		$('#kode_sasaran_e2<?=$objectId;?>').val(kode);
		searchData<?=$objectId?>();
	}
	...
	searchData<?=$objectId;?> = function (){
				...
				var sasaran = $("#kode_sasaran_e2<?=$objectId;?>").val();
				...
				$('#dg<?=$objectId;?>').datagrid({
					url:"<?=base_url()?>dashboard/dsb_capaian_e2/grid/"+filtahun+"/"+sasaran,
