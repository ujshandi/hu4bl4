	<script  type="text/javascript" >
		$(function(){
			...
			newData<?=$objectId;?> = function (){  			
				addTab("Add RKT Eselon I","rencana/rkteselon1/add");
			}
			//end newData 
			...
			editData<?=$objectId;?> = function (editmode){ 
				...
				if (row){
					if(editmode){
						if(row.status == '0'){
							addTab("Edit RKT Eselon I", "rencana/rkteselon1/edit/"+row.id_rkt_e1);
						}else{
							alert('Maaf data tidak bisa diedit, karena sudah ditetapkan di PK.');
						}
					}else{
						addTab("View RKT Eselon I", "rencana/rkteselon1/edit/"+row.id_rkt_e1+"/"+editmode);
					}
			...
			getUrl<?=$objectId;?> = function (tipe){
				...
				if (tipe==1){
					return "<?=base_url()?>rencana/rkteselon1/grid/"+filtahun+"/"+file1;
				}
				else if (tipe==2){
					return "<?=base_url()?>rencana/rkteselon1/pdf/"+filtahun+"/"+file1;
				}else if (tipe==3){
					return "<?=base_url()?>rencana/rkteselon1/excel/"+filtahun+"/"+file1;
				}
			}
			...
			setTimeout(function(){
				searchData<?=$objectId;?> ();
				//$('#dg<?=$objectId;?>').datagrid({url:"<?=base_url()?>rencana/rkteselon1/grid"});
			},50);
		...
		<tr>
			...
			<div class="fsearch"?>
				...
				<tr>
					<td>Tahun :</td>
					<td>
					<?=$this->rkteselon1_model->getListFilterTahun($objectId)?>
					</td>
				</tr>
				<tr>
	...
	<table id="dg<?=$objectId;?>" class="easyui-datagrid" style="height:auto;width:auto" title="Data Rencana Kerja Tahunan (RKT) Eselon I" toolbar="#tb<?=$objectId;?>" fitColumns="true" singleSelect="true" rownumbers="true" pagination="true">
	  <thead>
	  <tr>
