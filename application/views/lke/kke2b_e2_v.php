	<script  type="text/javascript" >
		$(function(){
			...
			clearFilter<?=$objectId;?> = function (){
				...
				//$('#dg<?=$objectId;?>').datagrid({url:"<?=base_url()?>lke/kke2b_e2/grid/"+filtahun+"/"+filnama+"/"+filalamat});
					//$('#dg<?=$objectId;?>').datagrid({url:"<?=base_url()?>lke/kke2b_e2/grid/"+filtahun+"/"+filnama+"/"+filalamat});
			}
			...
			getUrl<?=$objectId;?> = function (tipe){
				...
				if (tipe==1){
					return "<?=base_url()?>lke/kke2b_e2/grid/"+filtahun+"/"+file2;
				}
				else if (tipe==2){
					return "<?=base_url()?>lke/kke2b_e2/pdf/"+filtahun+"/"+file2+"/"+filsasaran+"/"+filiku+paging;
				}else if (tipe==3){
					return "<?=base_url()?>lke/kke2b_e2/excel/"+filtahun+"/"+file2+"/"+filsasaran+"/"+filiku+paging;
				}
			...
			newData<?=$objectId;?> = function (){  
				...
				if (row){
					...
					if (row){
						...
						//initCombo<?=$objectId?>();
						url = base_url+'lke/kke2b_e2/save';  
						$("#kke2b_e2_id<?=$objectId?>").val(row.kke2b_e2_id);
				...
				//addTab("Add PK Kementerian", "lke/kke2b_e2/add");
			}
			...
			setTimeout(function(){
				/* $('#dg<?=$objectId;?>').datagrid({
				url:"<?=base_url()?>lke/kke2b_e2/grid",
				queryParams:{lastNo:'0'},		
					onLoadSuccess:function(data){
						$('#dg<?=$objectId;?>').datagrid('options').queryParams.lastNo = data.lastNo;
						prepareMerge<?=$objectId;?>(data);
	...
	<div id="tb<?=$objectId;?>" style="height:auto">
	  ...
	  <? if($this->sys_menu_model->cekAkses('ADD;',319,$this->session->userdata('group_id'),$this->session->userdata('level_id'))){?>
	...
	<table id="dg<?=$objectId;?>" style="height:auto;width:auto" title="Laporan KKE2B Sasaran Eselon II" toolbar="#tb<?=$objectId;?>" fitColumns="false" singleSelect="true" rownumbers="false" pagination="true"  nowrap="false" showFooter="true">
	  <thead>
	  <tr>
		
		<th field="no" rowspan="4" sortable="false"  halign="center" align="center"  width="25px">No.</th>
		<th field="kke2b_e2_id" rowspan="4" sortable="false" hidden="true" width="25px">kke2b_e2_id</th>
	...
	<div id="dlg<?=$objectId;?>" class="easyui-dialog" style="width:720px;height:350px;padding:10px 20px" closed="true"  buttons="#dlg-buttons">
		...
		<form id="fm<?=$objectId;?>" method="post" >			
			...
			<div class="fitem">
				...
				<input type="hidden" id="kke2b_e2_id<?=$objectId?>" name="kke2b_e2_id"/>				
			</div>
			<div class="fitem">
