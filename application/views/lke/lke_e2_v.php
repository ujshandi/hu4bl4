<script  type="text/javascript" >
		...
		$(function(){
			...
			newData<?=$objectId;?> = function (){  
				...
				if (row){
					...
					$('#dlg<?=$objectId;?>').dialog('open').dialog('setTitle','Add LKE Eselon II');  
					$('#fm<?=$objectId;?>').form('clear');  
					idLke_e2<?=$objectId;?> = row.id;
					url = base_url+'lke/lke_e2/save';  
					
					$("#deskripsi_komponen<?=$objectId?>").load(base_url+'lke/lke_e2/loadtreeup/'+row.komponen_id);
					//$("#lke_e2_id<?=$objectId?>").val(row.lke_e2_id);
					$("#tahun<?=$objectId?>").val($("#filter_tahun<?=$objectId?>").val());	
					$("#id_komponen<?=$objectId?>").val(row.komponen_id);
					$("#lke_e2_id<?=$objectId?>").val("");
					$("#ref<?=$objectId?>").val(row.ref);
					$("#kode_e1<?=$objectId?>").val('<?=FILTER_E1?>');
					
					setListE2<?=$objectId?>();
					$("#kode_e2<?=$objectId?>").val($("#filter_e2<?=$objectId?>").val());
					<? $i=0;
				...
				//addTab("Add PK Kementerian", "lke/lke_e2/add");
			}
			//end newData 
			...
			getUrl<?=$objectId;?> = function (tipe){
				...
				if((e2==null)||(e2.length==0)) e2 ="-1";
				
				if (tipe==1){
					return "<?=base_url()?>lke/lke_e2/tree/"+filtahun+"/"+e2;
				}
				else if (tipe==2){
					return "<?=base_url()?>lke/lke_e2/pdf/"+filtahun+"/"+e2;
				}else if (tipe==3){
					return "<?=base_url()?>lke/lke_e2/excel/"+filtahun+"/"+e2;
				}
	...
	<div id="tb<?=$objectId;?>" style="height:auto">
	   <table border="0" cellpadding="1" cellspacing="1" width="100%">
		<tr>
			<td>
			<div class="fsearch" <?=($this->session->userdata('unit_kerja_e2')=='-1'?'':'')?>>
				<table border="0" cellpadding="1" cellspacing="1">
				<tr>
					<td>Tahun :</td>
					<td>
					<?=$this->lke_e2_model->getListTahun($objectId)?>
					</td>
				</tr>
				<tr>
	  ...
	  <div style="margin-bottom:5px">
		...
		<? if($this->sys_menu_model->cekAkses('ADD;',315,$this->session->userdata('group_id'),$this->session->userdata('level_id'))){?>
	...
	<div id="dlg<?=$objectId;?>" class="easyui-dialog" style="width:720px;height:400px;padding:10px 20px" closed="true"  buttons="#dlg-buttons">
		...
		<form id="fm<?=$objectId;?>" method="post" >			
			<div class="fitem">
				...
				<input type="hidden" id="lke_e2_id<?=$objectId?>" name="lke_e2_id"/>
			...
			<div class="fitem" >
					...
					<div class="fitem">							
						<label style="width:130px">Unit Kerja Eselon II :</label>
