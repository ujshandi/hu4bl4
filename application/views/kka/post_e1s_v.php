	<script  type="text/javascript" >
		$(function(){
			...
			newData<?=$objectId;?> = function (){  			
				addTab("Add Post Monev Eselon I","kka/post_e1/add");
			}
			//end newData 
			...
			editData<?=$objectId;?> = function (editmode){ 
				...
				if (row){
					if(editmode){
						if(row.status == '0'){
							addTab("Edit Post Monev Eselon I", "kka/post_e1/edit/"+row.poste1_id);
						}else{
							alert('Maaf data tidak bisa diedit, karena sudah ditetapkan di PK.');
						}
					}else{
						addTab("View Post Monev Eselon I", "kka/post_e1/edit/"+row.poste1_id+"/"+editmode);
					}
				...
				<? if ($this->session->userdata('unit_kerja_e1')=='-1'){?>				
					...
					if(row){
						if(confirm("Apakah yakin akan menghapus data '" + row.kode_iku_e1 + "'?")){
							var response = '';
							$.ajax({ type: "GET",
									 url: base_url+'kka/post_e1/delete/' + row.poste1_id,
									 async: false,
			...
			getUrl<?=$objectId;?> = function (tipe){
				...
				if (tipe==1){
					return "<?=base_url()?>kka/post_e1/gridKegiatan/"+filtahun+"/"+file1;
				}
				else if (tipe==2){
					return "<?=base_url()?>kka/post_e1/pdf/"+filtahun+"/"+file1;
				}else if (tipe==3){
					return "<?=base_url()?>kka/post_e1/excel/"+filtahun+"/"+file1;
				}
			}
			...
			setTimeout(function(){
				searchData<?=$objectId;?> ();
				//$('#dg<?=$objectId;?>').datagrid({url:"<?=base_url()?>kka/post_e1/grid"});
			},50);
...
<table id="dg<?=$objectId;?>" style="height:auto;width:auto" title="Data Post Monev Eselon I" toolbar="#tb<?=$objectId;?>" fitColumns="true" singleSelect="true" rownumbers="true" pagination="true" nowrap="false">
	<thead>
	<tr>
			...
			$('#dg<?=$objectId;?>').datagrid({
				...
				//	alert(row.id_pk_e1);
                    ...
                    $('#ddv<?=$objectId;?>-'+index).datagrid({
                        url:'<?=base_url()?>kka/post_e1/gridMonev/'+row.tahun+'/'+row.kode_kegiatan+'/?parentIndex='+index,
                        fitColumns:true,
                        singleSelect:true,
                        rownumbers:true,
						nowrap:false,
                        ...
                        height:'auto',		
                        columns:[[
                            {field:'poste1_id',title:'id',hidden:true},
...
<!--	<table id="dg<?=$objectId;?>" style="height:auto;width:auto" title="Data Pre Definitif Eselon I" toolbar="#tb<?=$objectId;?>" fitColumns="true" singleSelect="true" rownumbers="true" pagination="true" noWrap="false">
	  <thead>
	  <tr>
		<th field="poste1_id" sortable="true" hidden="true" width="50px">poste1_id</th>
