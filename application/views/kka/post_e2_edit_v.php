	...
	<script  type="text/javascript" >
		...
		$(function(){
			...
			cancel<?=$objectId;?>=function(){
				$('#tt').tabs('close', '<?=($editmode==TRUE?'Edit':'View')?> Data Post Monev Eselon II');
			}
			
		 	saveDataEdit<?=$objectId;?>=function(){
				$('#fmedit<?=$objectId;?>').form('submit',{
					url: base_url+'kka/post_e2/save_edit',
					onSubmit: function(){
					...
					success: function(result){
						...
						if (result.success){
							...
							$('#tt').tabs('close', 'Edit Data Post Monev Eselon II');
						} else {
							$.messager.show({
								title: 'Error',
								msg: result.msg
	...
	<div id="cc<?=$objectId;?>" class="easyui-layout" fit="true">  
		...
		<div region="north" split="true" title="" style="height:450px;">
			<div class="easyui-layout" fit="true">  
				...
				<div region="center" border="true" title="Edit Data Post Monev Eselon II">	
					<form id="fmedit<?=$objectId;?>" method="post" style="margin:10px 5px 5px 10px;">
						<input name="poste2_id" class="" type="hidden" value="<?=$result->poste2_id?>">
						<div class="fitem">
						...
						<div class="fitem">
							<label style="width:150px">Anggaran :</label>
							<input name="anggaran" class="easyui-validatebox money" required="true" style="text-align:right" size="20" value="<?=$result->anggaran;?>">
