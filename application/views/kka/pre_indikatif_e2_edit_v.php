	...
	<script  type="text/javascript" >
		...
		$(function(){
			...
			cancel<?=$objectId;?>=function(){
				$('#tt').tabs('close', '<?=($editmode==TRUE?'Edit':'View')?> Pra Monev Eselon II Pagu Indikatif');
			}
			
		 	saveDataEdit<?=$objectId;?>=function(){
				$('#fmedit<?=$objectId;?>').form('submit',{
					url: base_url+'kka/pre_indikatif_e2/save_edit',
					onSubmit: function(){
					...
					success: function(result){
						...
						if (result.success){
							...
							$('#tt').tabs('close', 'Edit Pra Monev Eselon II Pagu Indikatif');
						} else {
							$.messager.show({
								title: 'Error',
								msg: result.msg
	...
	<div id="cc<?=$objectId;?>" class="easyui-layout" fit="true">  
		...
		<div region="north" split="true" title="" style="height:450px;">
			<div class="easyui-layout" fit="true">  
				...
				<div region="center" border="true" title="Edit Pra Monev Eselon II Pagu Indikatif">	
					<form id="fmedit<?=$objectId;?>" method="post" style="margin:10px 5px 5px 10px;">
						<input name="preindikatif_e2_id" class="" type="hidden" value="<?=$result->preindikatif_e2_id?>">
						<div class="fitem">
