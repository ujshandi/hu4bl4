	...
	<script  type="text/javascript" >
		$(function(){
			...
			getUrl<?=$objectId;?> = function (tipe){
				...
				if (tipe==1){
					return "<?=base_url()?>realisasi/rpt_capaian_kinerjae2/grid/"+filtahun+"/"+filsasaran+"/"+filiku+"/"+file1+"/"+file2+"/"+filstart+"/"+filend;
				}
				else if (tipe==2){
					return "<?=base_url()?>realisasi/rpt_capaian_kinerjae2/pdf/"+filtahun+"/"+filsasaran+"/"+filiku+"/"+file1+"/"+file2+"/"+filstart+"/"+filend+paging;
				}else if (tipe==3){
					return "<?=base_url()?>realisasi/rpt_capaian_kinerjae2/excel/"+filtahun+"/"+filsasaran+"/"+filiku+"/"+file1+"/"+file2+"/"+filstart+"/"+filend+paging;
				}
			...
			setTimeout(function(){
				searchData<?=$objectId;?>();
			/* 	$('#dg<?=$objectId;?>').datagrid({
					url:"<?=base_url()?>realisasi/rpt_capaian_kinerjae2/grid/",
					queryParams:{lastNo:'0'},	
					onLoadSuccess:function(data){
						$('#dg<?=$objectId;?>').datagrid('options').queryParams.lastNo = data.lastNo;
						//prepareMerge<?=$objectId;?>(data);
	...
	<div id="tb<?=$objectId;?>" style="height:auto">
	  ...
	  <tr>
		<td>
		  <div class="fsearch" style="">
			...
			<tr>
				...
				<td><?=$this->rpt_capaian_kinerjae2_model->getListTahun($objectId)?></td>
			</tr>		
			<tr>
				<td>Bulan dari :</td>
			...
			<tr>
				<td style="width:120px">Unit Kerja Eselon I</td>
				<td colspan="3">
					<?=$this->eselon1_model->getListFilterEselon1($objectId,$this->session->userdata('unit_kerja_e1'))?>
				</td>
			</tr>
			<tr>
				<td style="width:120px">Unit Kerja Eselon II</td>
				<td colspan="3"><span class="fitem" id="divEselon2<?=$objectId;?>">
	   ...
	   <div style="margin-bottom:5px">  
			<? if($this->sys_menu_model->cekAkses('PRINT;',268,$this->session->userdata('group_id'),$this->session->userdata('level_id'))){?>
				<a 	href="#" onclick="printData<?=$objectId;?>();" class="easyui-linkbutton" iconCls="icon-print" plain="true">Print</a>
			<?}?>
			<? if($this->sys_menu_model->cekAkses('EXCEL;',268,$this->session->userdata('group_id'),$this->session->userdata('level_id'))){?>
				<a href="#" onclick="toExcel<?=$objectId;?>();" class="easyui-linkbutton" iconCls="icon-excel" plain="true">Excel</a>
			<?}?>
	  </div>
	</div>
	
	<table id="dg<?=$objectId;?>"  style="height:auto;width:auto" title="<?=$title?>" toolbar="#tb<?=$objectId;?>" fitColumns="false" singleSelect="true" rownumbers="false" pagination="true" nowrap="false" showFooter="true">
	  <thead>
	 <tr>
		
		<th field="no" sortable="false" rowspan="2" width="25px">No.</th>
		<th field="indikator_kinerja"  rowspan="2"  sortable="false" width="400px">Indikator Kinerja Kegiatan</th>
