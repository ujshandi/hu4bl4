	<script  type="text/javascript" >
		$(function(){
			...
			getUrl<?=$objectId;?> = function (tipe){
				...
				if (tipe==1){
					return "<?=base_url()?>realisasi/rpt_capaian_kinerjae1/grid/"+filtahun+"/"+filsasaran+"/"+filiku+"/"+file1+"/"+filstart+"/"+filend;
				}
				else if (tipe==2){
					return "<?=base_url()?>realisasi/rpt_capaian_kinerjae1/pdf/"+filtahun+"/"+filsasaran+"/"+filiku+"/"+file1+"/"+filstart+"/"+filend+paging;
				}else if (tipe==3){
					return "<?=base_url()?>realisasi/rpt_capaian_kinerjae1/excel/"+filtahun+"/"+filsasaran+"/"+filiku+"/"+file1+"/"+filstart+"/"+filend+paging;
				}
			...
			setTimeout(function(){
				/* $('#dg<?=$objectId;?>').datagrid({
					url:"<?=base_url()?>realisasi/rpt_capaian_kinerjae1/grid",
					queryParams:{lastNo:'0'},	
					onLoadSuccess:function(data){
						$('#dg<?=$objectId;?>').datagrid('options').queryParams.lastNo = data.lastNo;
						//prepareMerge<?=$objectId;?>(data);
	...
	<div id="tb<?=$objectId;?>" style="height:auto">
	  ...
	  <tr>
		<td>
		  <div class="fsearch" style="">
			...
			<tr>
				...
				<td><?=$this->rpt_capaian_kinerjae1_model->getListTahun($objectId)?></td>
			</tr>	
			<tr>
				<td>Bulan dari :</td>
	   ...
	   <div style="margin-bottom:5px">  
			<? if($this->sys_menu_model->cekAkses('PRINT;',262,$this->session->userdata('group_id'),$this->session->userdata('level_id'))){?>
				<a 	href="#" onclick="printData<?=$objectId;?>();" class="easyui-linkbutton" iconCls="icon-print" plain="true">Print</a>
			<?}?>
			<? if($this->sys_menu_model->cekAkses('EXCEL;',262,$this->session->userdata('group_id'),$this->session->userdata('level_id'))){?>
