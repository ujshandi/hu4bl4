	...
	<script  type="text/javascript" >
		...
		$(function(){
			...
			cancel<?=$objectId;?>=function(){
				$('#tt').tabs('close', 'Pengesahan PK Kementerian');
			}
			
		 	saveData<?=$objectId;?>=function(){
				$('#fm<?=$objectId;?>').form('submit',{
					url: base_url+'penetapan/pengesahan_penetapankl/save',
					onSubmit: function(){
	...
	<script language="javascript">
		...
		function getDetail<?=$objectId;?>(){
			...
			$.ajax({ type: "GET",   
					 url: base_url+'penetapan/pengesahan_penetapankl/getDetail/' + tahun + '/' + kode_kl,
					 async: false,
	...
	<div id="cc<?=$objectId;?>" class="easyui-layout" fit="true">  
				...
				<div region="center" border="true" title="Pengesahan Data Penetapan Kinerja Kementerian">
					<form id="fm<?=$objectId;?>" method="post" style="margin:10px 5px 5px 10px;">		
						<div class="fitem">
						  <label style="width:120px" >Tahun :</label>
						  <?=$this->penetapankl_model->getListTahun($objectId)?>
						</div>
						<div class="fitem">
