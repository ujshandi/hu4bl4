	<script  type="text/javascript" >
		$(function(){
			...
			newData<?=$objectId;?> = function (){  
				$('#dlg<?=$objectId;?>').dialog('open').dialog('setTitle','Add Program PK Kementerian');  
				$('#fm<?=$objectId;?>').form('clear');  
				url = base_url+'penetapan/masterpenetapankl/save/add';  
			...
			editData<?=$objectId;?> = function (){
				...
				if (row){
					$('#dlg<?=$objectId;?>').dialog('open').dialog('setTitle','Edit Program PK Kementerian');
					$('#fm<?=$objectId;?>').form('load', row);
					url = base_url+'penetapan/masterpenetapankl/save/edit/'+row.id_masterpk_kl;//+row.id;//'update_user.php?id='+row.id;
				}
				...
				<? if ($this->session->userdata('unit_kerja_e1')=='-1'){?>				
					...
					if(row){
						if(confirm("Apakah yakin akan menghapus data '" + row.kode_program + "'?")){
							var response = '';
							$.ajax({ type: "GET",
									 url: base_url+'penetapan/masterpenetapankl/delete/' + row.id_masterpk_kl,
									 async: false,
			...
			getUrl<?=$objectId;?> = function (tipe){
				...
				if (tipe==1){
					return "<?=base_url()?>penetapan/masterpenetapankl/grid/"+filtahun;
				}
				else if (tipe==2){
					return "<?=base_url()?>penetapan/masterpenetapankl/pdf/"+filtahun;
				}else if (tipe==3){
					return "<?=base_url()?>penetapan/masterpenetapankl/excel/"+filtahun;
				}
			...
			setTimeout(function(){
				$('#dg<?=$objectId;?>').datagrid({url:"<?=base_url()?>penetapan/masterpenetapankl/grid"});
			},0);
		 });
	</script>
	<style type="text/css">
		...
		<tr>
			...
			<div class="fsearch" <?=($this->session->userdata('unit_kerja_e1')=='-1'?'':'style="display:none"')?>>
				...
				<tr>
					<td>Tahun :</td>
					<td>
					<?=$this->master_penetapankl_model->getListFilterTahun($objectId)?>
					</td>
				</tr>
	...
	<table id="dg<?=$objectId;?>" class="easyui-datagrid" style="height:auto;width:auto" title="Data Program Penetapan Kinerja (PK) Kementerian" toolbar="#tb<?=$objectId;?>" fitColumns="true" singleSelect="true" rownumbers="true" pagination="true">
	  <thead>
	  <tr>
		<th field="id_masterpk_kl" hidden="true" sortable="true" width="50px">Kode</th>
		...
		<th field="nama_program" sortable="true" width="100px">Nama Program</th>
	...
	<div id="dlg<?=$objectId;?>" class="easyui-dialog" style="width:850px;height:250px;padding:10px 20px" closed="true" buttons="#dlg-buttons">
		<div class="ftitle">Tambah/Edit Data Program Penetapan Kinerja (PK) Kementerian</div>
		<form id="fm<?=$objectId;?>" method="post">		
			<div class="fitem">
			  <label style="width:120px">Tahun :</label>
			  <?=$this->penetapankl_model->getListTahun()?>
			</div>					
			<div class="fitem">
