...
class Lke_e2_model extends CI_Model
{	
	/**
	* constructor
	*/
	...
	public function easyGrid($filtahun=null,$file1=null){
		...
		$sort = isset($_POST['sort']) ? strval($_POST['sort']) : 'tbl_lke_e2.tahun';  
		...
		if ($count>0){
			...
			if($filtahun != '' && $filtahun != '-1' && $filtahun != null) {
				$this->db->where("tbl_lke_e2.tahun",$filtahun);
			}
			if($file1 != '' && $file1 != '-1' && $file1 != null) {
				$this->db->where("tbl_lke_e2.kode_e1",$file1);
			}
			$this->db->order_by($sort." ".$order );
			$this->db->limit($limit,$offset);
			//$this->db->select("*, tbl_lke_e2.kode_e1 as pk_kode_e1",false);
			$this->db->select("tbl_lke_e2.*",false);
			$this->db->from('tbl_lke_e2');			
			$query = $this->db->get();
			
			$i=0;
			foreach ($query->result() as $row)
			{
				$response->rows[$i]['lke_e2_id']=$row->lke_e2_id;
	...
	public function loadTree($parentId=null){
		$response = new stdClass();	
			...
			//$this->db->limit($limit,$offset);
			//$this->db->select("*, tbl_lke_e2.kode_e1 as pk_kode_e1",false);
			$this->db->select("tbl_lke_e2.*,tbl_komponen_lke.nama_komponen,tbl_komponen_lke.id_komponen as komponen_id",false);
			$this->db->from('tbl_komponen_lke');			
			//$this->db->join('tbl_lke_e2_detail',"tbl_komponen_lke.id_komponen=tbl_lke_e2_detail.id_komponen","left");			
			$this->db->join('tbl_lke_e2',"tbl_komponen_lke.id_komponen = tbl_lke_e2.id_komponen","left");			
			$this->db->join('tbl_eselon2',"tbl_eselon2.kode_e2 = tbl_lke_e2.kode_e2","left");			
			$query = $this->db->get();
			$result = array();
			$i=0;
			foreach ($query->result() as $row)
			{
				...
				$response->rows[$i]['lke_e2_id']=($row->lke_e2_id==null?"":$row->lke_e2_id);
	...
	public function loadTreeUp($id=null,& $result="",$level=0){
			...
			$this->db->from('tbl_komponen_lke');			
			//$this->db->join('tbl_lke_e2_detail',"tbl_komponen_lke.id_komponen=tbl_lke_e2_detail.id_komponen","left");			
			//$this->db->join('tbl_lke_e2',"tbl_lke_e2.lke_e2_id=tbl_lke_e2_detail.lke_e2_id","left");			
			$query = $this->db->get();
	...
	public function InsertOnDb($data,& $error) {
		//query insert data		
		$this->db->set('lke_e2_id',$data['lke_e2_id']);
		...
		$result = $this->db->insert('tbl_lke_e2');
	...
	public function UpdateOnDb($data, $kode) {
		...
		$result=$this->db->update('tbl_lke_e2');
	...
	public function isExistNilai($tahun,$kode_e2,$id_komponen){	
	...
	private function treeHasChild($id){
		$this->db->from('tbl_komponen_lke');
		$this->db->where('induk',$id);
		//$this->db->join('tbl_lke_e2_detail', 'tbl_lke_e2_detail.kode_iku_e1 = tbl_lke_e2.kode_iku_e1 and tbl_lke_e2_detail.tahun = tbl_lke_e2.tahun','left');
		//$this->db->join('tbl_sasaran_eselon1', 'tbl_sasaran_eselon1.kode_sasaran_e1 = tbl_lke_e2.kode_sasaran_e1 and tbl_sasaran_eselon1.tahun = tbl_lke_e2.tahun','left');
	...
	//	$response->total = $count;
		...
		//if ($count>0){
			//filter
				...
				$this->db->where("d.lke_e2_id",$id_pk);
			
			
			//$this->db->order_by($sort." ".$order );
		//	$this->db->limit($limit,$offset);
			$this->db->select("distinct d.*,k.nama_komponen",false);
			$this->db->from('tbl_lke_e2_detail d');
			$this->db->join('tbl_komponen_lke k','d.id_komponen=k.id_komponen');
			
			$this->db->order_by("tbl_lke_e2_detail.id_komponen");
			$query = $this->db->get();
	...
	public function DeleteOnDb($id){
		...
		$result = $this->db->insert('tbl_lke_e2_log');
		...
		$this->db->flush_cache();
		$this->db->where('lke_e2_id', $id);
		$result = $this->db->delete('tbl_lke_e2'); 
