...
class Kke3a_e2_model extends CI_Model
{	
	/**
	* constructor
	*/
	...
	public function easyGrid($filtahun=null,$file2=null,$filsasaran=null,$filiku=null,$purpose=1,$pageNumber=null,$pageSize=null){
		...
		if ($count>0){
			...
			$this->db->select("rkt.kode_sasaran_e2,sasaran_strategis.deskripsi as sasaran_strategis, iku.deskripsi as indikator_kinerja,rkt.tahun,rkt.kode_ikk,lke.kke3a_e2_id,lke.renstra_ip,lke.renstra_ip_nilai, lke.rkt_ip,lke.rkt_ip_nilai, lke.pk_ip, lke.pk_ip_nilai, lke.iku_measurable,lke.iku_measurable_nilai, lke.iku_hasil, lke.iku_hasil_nilai,lke.iku_relevan, lke.iku_relevan_nilai,lke.iku_diukur, lke.iku_diukur_nilai,lke.kriteria_measurable, lke.kriteria_measurable_nilai,lke.kriteria_hasil, lke.kriteria_hasil_nilai, lke.kriteria_relevan, lke.kriteria_relevan_nilai, lke.kriteria_diukur, lke.kriteria_diukur_nilai, lke.pengukuran, lke.pengukuran_nilai,rkt.kode_e2,e2.nama_e2",false);
			$this->db->from('tbl_rkt_eselon2 rkt inner join tbl_ikk iku on iku.kode_ikk = rkt.kode_ikk and rkt.tahun = iku.tahun inner join tbl_sasaran_eselon2 sasaran_strategis on sasaran_strategis.kode_sasaran_e2 = rkt.kode_sasaran_e2 and sasaran_strategis.tahun=rkt.tahun left join tbl_kke3a_e2 lke on rkt.kode_sasaran_e2=lke.kode_sasaran_e2 and rkt.tahun=lke.tahun and rkt.kode_ikk=lke.kode_ikk inner join tbl_eselon2 e2 on rkt.kode_e2=e2.kode_e2', false);
			$query = $this->db->get();
			...
			foreach ($query->result() as $row)
			{
				...
				$response->rows[$i]['kke3a_e2_id']=$row->kke3a_e2_id;
				...
				//$this->getIndex('pk_ip',$row->tahun,$row->kode_sasaran_e2,$row->kode_ikk);//$this->utility->cekNumericFmt($row->target);
...
//utk kepentingan export excel ==========================
		...
		//$this->db->from('tbl_kke3a_e2');
		//$this->db->select("select sasaran.deskripsi as sasaran_srategis, iku.deskripsi as indikator_kinerja, rkt.target",false);
	//	$this->db->from('tbl_kke3a_e2 kke inner join tbl_ikk iku on kke.kode_ikk = iku.kode_ikk and kke.tahun=iku.tahun inner join tbl_sasaran_eselon2 sasaran on sasaran.kode_sasaran_e2 = iku.kode_sasaran_e2 and sasaran.tahun=iku.tahun', false);
		$this->db->from('tbl_rkt_eselon2 rkt inner join tbl_ikk iku on iku.kode_ikk = rkt.kode_ikk and rkt.tahun = iku.tahun
inner join tbl_sasaran_eselon2 sasaran on sasaran.kode_sasaran_e2 = rkt.kode_sasaran_e2 and sasaran.tahun=rkt.tahun left join tbl_kke3a_e2 lke on rkt.kode_sasaran_e2=lke.kode_sasaran_e2 and rkt.tahun=lke.tahun and rkt.kode_ikk=lke.kode_ikk', false);
	...
	public function InsertOnDb($data,& $error) {
		...
		$result = $this->db->insert('tbl_kke3a_e2');
	...
	public function UpdateOnDb($data, $kode) {
		...
		$this->db->where('kke3a_e2_id',$kode);
		...
		$result=$this->db->update('tbl_kke3a_e2');
