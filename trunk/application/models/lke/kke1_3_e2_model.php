...
class Kke1_3_e2_model extends CI_Model
{	
	/**
	* constructor
	*/
	...
	public function easyGrid($filtahun=null,$file1=null,$filsasaran=null,$filiku=null,$purpose=1,$pageNumber=null,$pageSize=null){
		...
		if ($count>0){
			...
			/* if($file1 != '' && $file1 != '-1' && $file1 != null) {
						$this->db->where("kke.kode_e2",$file1);
			} */
			...
			$this->db->select("rkt.kode_sasaran_e2,sasaran_strategis.deskripsi as sasaran_strategis, iku.deskripsi as indikator_kinerja,rkt.tahun,rkt.kode_ikk,lke.catatan_keuangan,lke.catatan_keuangan_nilai, lke.masyarakat,lke.masyarakat_nilai, lke.instansi_lainnya, lke.instansi_lainnya_nilai, lke.transparansi,lke.transparansi_nilai, lke.penghargaan, lke.penghargaan_nilai,lke.kke13_e2_id,rkt.kode_e2, e2.nama_e2",false);
			$this->db->from('tbl_rkt_eselon2 rkt inner join tbl_ikk iku on iku.kode_ikk = rkt.kode_ikk and rkt.tahun = iku.tahun inner join tbl_sasaran_eselon2 sasaran_strategis on sasaran_strategis.kode_sasaran_e2 = rkt.kode_sasaran_e2 and sasaran_strategis.tahun=rkt.tahun left join tbl_kke1_3_e2 lke on rkt.kode_sasaran_e2=lke.kode_sasaran_e2 and rkt.tahun=lke.tahun and rkt.kode_ikk=lke.kode_ikk  inner join tbl_eselon2 e2 on rkt.kode_e2=e2.kode_e2', false);
			$query = $this->db->get();
			...
			foreach ($query->result() as $row)
			{
				...
				$response->rows[$i]['kke13_e2_id']=$row->kke13_e2_id;
				...
				//$this->getIndex('instansi_lainnya',$row->tahun,$row->kode_sasaran_e2,$row->kode_ikk);//$this->utility->cekNumericFmt($row->target);
...
//utk kepentingan export excel ==========================
	...
	public function InsertOnDb($data,& $error) {
		...
		$result = $this->db->insert('tbl_kke1_3_e2');
	...
	public function UpdateOnDb($data, $kode) {
		...
		$this->db->where('kke13_e2_id',$kode);
		...
		$result=$this->db->update('tbl_kke1_3_e2');
	...
	public function GetRecordCount($filtahun=null,$file1=null,$filsasaran=null,$filiku=null){
		...
		/* if($file1 != '' && $file1 != '-1' && $file1 != null) {
					$this->db->where("rkt.kode_e2",$file1);
		} */
		if($filsasaran != '' && $filsasaran != '-1' && $filsasaran != null) {
				$this->db->where("rkt.kode_sasaran_e2",$filsasaran);
		}
	...
	//	$this->db->from('tbl_kke1_3_e2 kke inner join tbl_ikk iku on kke.kode_ikk = iku.kode_ikk and kke.tahun=iku.tahun inner join tbl_sasaran_eselon2 sasaran on sasaran.kode_sasaran_e2 = iku.kode_sasaran_e2 and sasaran.tahun=iku.tahun', false);
		$this->db->from('tbl_rkt_eselon2 rkt inner join tbl_ikk iku on iku.kode_ikk = rkt.kode_ikk and rkt.tahun = iku.tahun
inner join tbl_sasaran_eselon2 sasaran on sasaran.kode_sasaran_e2 = rkt.kode_sasaran_e2 and sasaran.tahun=rkt.tahun left join tbl_kke1_3_e2 lke on rkt.kode_sasaran_e2=lke.kode_sasaran_e2 and rkt.tahun=lke.tahun and rkt.kode_ikk=lke.kode_ikk', false);
