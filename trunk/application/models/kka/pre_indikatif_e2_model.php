...
class Pre_indikatif_e2_model extends CI_Model
{	
	/**
	* constructor
	*/
	...
	public function easyGrid($filtahun=null,$file2=null,$purpose=1){
		...
		if ($count>0){
			...
			$this->db->select('distinct a.preindikatif_e2_id, a.tahun, a.kode_ikk, a.kode_e2 as rkt_kode_e2, a.kode_sasaran_e2 AS kode_sasaran_e2, b.deskripsi, a.jumlah,k.kode_kegiatan,k.nama_kegiatan,subkl.kode_subkegiatan, subkl.nama_subkegiatan',false);
			$this->db->select("c.deskripsi as deskripsi_sasaran_e1, b.deskripsi AS deskripsi_iku_e1, d.nama_e2");
			$this->db->from('tbl_pre_indikatif_e2 a');
			...
			foreach ($query->result() as $row)
			{
				...
				$response->rows[$i]['preindikatif_e2_id']=$row->preindikatif_e2_id;
	...
	public function getDataEdit($id){
		...
		$this->db->where('a.preindikatif_e2_id', $id);
	...
	public function InsertOnDB($data) {
		...
		foreach($data['detail'] as $dt){
			...
			$this->db->insert('tbl_pre_indikatif_e2');
			...
			$this->db->insert('tbl_pre_indikatif_e2_log'); */
		...
		try {
			$result = $this->db->insert('tbl_pre_indikatif_e2');
		}
		catch(Exception $e){
	...
	/* 	$this->db->select('k.*,pre.jumlah, pre.preusulan1_e2_id');
		...
		$this->db->join('tbl_pre_usulan1_e2 pre','k.kode_kegiatan=pre.kode_kegiatan and k.tahun=pre.tahun and pre.kode_e2=k.kode_e2','left');
		...
		foreach($que->result() as $r){
			...
			//if($e2!=''){$this->db->where('kode_e2',$e2);}
			$queSub = $this->db->get();
			$jumlah = $this->getJumlahSubkegiatan('tbl_pre_usulan1_e2',$r->tahun,$r->kode_kegiatan);
			//$this->db->flush_cache();
			$max_sub_idx = 0;
			if ($queSub!=null)
				$max_sub_idx=$queSub->num_rows;		
			//$checked = ((int)$r->preusulan1_e2_id>0?"checked":"");	
	...
	public function UpdateOnDb($data){
		$this->db->flush_cache();
		$this->db->where('preindikatif_e2_id', $data['preindikatif_e2_id']);
		
		//$this->db->set('kode_ikk', $data['kode_ikk']);
		$this->db->set('jumlah',$this->utility->ourDeFormatNumber2($data['jumlah']));
		$this->db->set('log_update', 		$this->session->userdata('user_id').';'.date('Y-m-d H:i:s'));
		
		$result = $this->db->update('tbl_pre_indikatif_e2');
		
		/* # insert to log
		$this->db->flush_cache();
		$this->db->select("*");
		$this->db->from("tbl_pre_indikatif_e2");
		$this->db->where('preindikatif_e2_id', $data['preindikatif_e2_id']);
		$qt = $this->db->get();
	...
	public function DeleteOnDb($id){
		...
		$this->db->select("*");
		$this->db->from("tbl_pre_indikatif_e2");
		$this->db->where('preindikatif_e2_id', $id);
		$qt = $this->db->get();
		...
		$this->db->flush_cache();
		$this->db->where('preindikatif_e2_id', $id);
		$result = $this->db->delete('tbl_pre_indikatif_e2'); 
	...
	public function getJumlahSubkegiatan($tableName,$tahun,$kodekegiatan){
		$this->db->flush_cache();
