...
class Pre_indikatif_e1_model extends CI_Model
{	
	/**
	* constructor
	*/
	...
	public function easyGrid($filtahun=null,$file1=null,$purpose=1){
		...
		if ($count>0){
			...
			$this->db->select('a.preindikatif_e1_id, a.tahun, a.kode_iku_e1, a.kode_e1 as rkt_kode_e1, a.kode_sasaran_e1 AS kode_sasaran_e12, b.deskripsi, a.jumlah,k.kode_kegiatan,k.nama_kegiatan');
			$this->db->select("c.deskripsi as deskripsi_sasaran_e1, b.deskripsi AS deskripsi_iku_e1, d.nama_e1");
			$this->db->from('tbl_pre_indikatif_e1 a');
	...
	public function easySubGrid($filtahun=null,$filkegiatan=null){
		...
		if ($count>0){
			...
			$this->db->order_by($sort." ".$order );
			//if($purpose==1){$this->db->limit($limit,$offset);}
			$this->db->select('a.preindikatif_e2_id, a.tahun, a.kode_ikk, a.kode_e2 as rkt_kode_e2, a.kode_sasaran_e2 AS kode_sasaran_e2, b.deskripsi, a.jumlah,skk.kode_subkegiatan,skk.nama_subkegiatan');
			...
			foreach ($query->result() as $row)
			{
				$no++;
				$response->rows[$i]['no']= $no;
				$response->rows[$i]['preusulan1_e1_id']=$row->preindikatif_e2_id;
				$response->rows[$i]['tahun']=$row->tahun;
