...
class Perubahaneselon1_model extends CI_Model
{	
	/**
	* constructor
	*/
	...
	//	var_dump($count);die;	
		...
		$sort = isset($_POST['sort']) ? strval($_POST['sort']) : 'tbl_historypk_e1.tahun';  
		$order = isset($_POST['order']) ? strval($_POST['order']) : 'desc';  
		$offset = ($page-1)*$limit;  
		
		if ($count>0){
			if($filidpk != '' && $filidpk != '-1' && $filidpk != null) {
				$this->db->where("tbl_historypk_e1.id_pk_e1",$filidpk);
			}	
			$this->db->order_by($sort." ".$order );
			$this->db->limit($limit,$offset);
			//$this->db->select("*, tbl_historypk_e1.kode_e1 as pk_kode_e1",false);
			$this->db->select("tbl_historypk_e1.*, tbl_iku_eselon1.deskripsi as deskripsi_iku_e1,tbl_iku_eselon1.satuan, tbl_sasaran_eselon1.deskripsi as deskripsi_sasaran_e1, tbl_eselon1.nama_e1",false);
			$this->db->from('tbl_historypk_e1');
			$this->db->join('tbl_iku_eselon1', 'tbl_iku_eselon1.kode_iku_e1 = tbl_historypk_e1.kode_iku_e1 and tbl_iku_eselon1.tahun = tbl_historypk_e1.tahun','left');
			$this->db->join('tbl_sasaran_eselon1', 'tbl_sasaran_eselon1.kode_sasaran_e1 = tbl_historypk_e1.kode_sasaran_e1 and tbl_sasaran_eselon1.tahun = tbl_historypk_e1.tahun','left');
			$this->db->join('tbl_eselon1', 'tbl_eselon1.kode_e1 = tbl_historypk_e1.kode_e1 ');
			$this->db->order_by("tbl_historypk_e1.tahun DESC, kode_sasaran_e1 ASC, tbl_historypk_e1.kode_iku_e1 ASC");
			$query = $this->db->get();
...
*/
	...
	public function GetRecordCount($filidpk){
		if($filidpk != '' && $filidpk != '-1' && $filidpk != null) {
			$this->db->where("tbl_historypk_e1.id_pk_e1",$filidpk);
		}	
		//$this->db->select('distinct tbl_historypk_e1.*, tbl_iku_eselon1.kode_e1 as pk_kode_e1');
		$this->db->from('tbl_historypk_e1');
			$this->db->join('tbl_iku_eselon1', 'tbl_iku_eselon1.kode_iku_e1 = tbl_historypk_e1.kode_iku_e1 and tbl_iku_eselon1.tahun = tbl_historypk_e1.tahun','left');
			$this->db->join('tbl_sasaran_eselon1', 'tbl_sasaran_eselon1.kode_sasaran_e1 = tbl_historypk_e1.kode_sasaran_e1 and tbl_sasaran_eselon1.tahun = tbl_historypk_e1.tahun','left');
			$this->db->join('tbl_eselon1', 'tbl_eselon1.kode_e1 = tbl_historypk_e1.kode_e1 ');
	...
	public function UpdateOnDb($data){
		...
		$this->db->flush_cache();
		$this->db->set('id_pk_e1',				$qt->row()->id_pk_e1);
		$this->db->set('no_history',	$this->getNoHistory($qt->row()->id_pk_e1));
		$this->db->set('tahun',				$qt->row()->tahun);
		$this->db->set('kode_e1',			$qt->row()->kode_e1);
		$this->db->set('kode_sasaran_e1',	$qt->row()->kode_sasaran_e1);
		$this->db->set('kode_iku_e1',		$qt->row()->kode_iku_e1);
		...
		$result = $this->db->insert('tbl_historypk_e1');
