	<script  type="text/javascript" >
		$(function(){
			...
			clearFilter<?=$objectId;?> = function (){
				...
				//$('#dg<?=$objectId;?>').datagrid({url:"<?=base_url()?>lke/kke2a_e2/grid/"+filtahun+"/"+filnama+"/"+filalamat});
					//$('#dg<?=$objectId;?>').datagrid({url:"<?=base_url()?>lke/kke2a_e2/grid/"+filtahun+"/"+filnama+"/"+filalamat});
			}
			...
			getUrl<?=$objectId;?> = function (tipe){
				...
				if (tipe==1){
					return "<?=base_url()?>lke/kke2a_e2/grid/"+filtahun+"/"+file2;
				}
				else if (tipe==2){
					return "<?=base_url()?>lke/kke2a_e2/pdf/"+filtahun+"/"+file2+"/"+filsasaran+"/"+filiku+paging;
				}else if (tipe==3){
					return "<?=base_url()?>lke/kke2a_e2/excel/"+filtahun+"/"+file2+"/"+filsasaran+"/"+filiku+paging;
				}
			...
			newData<?=$objectId;?> = function (){  
				...
				if (row){
					...
					if (row){
						...
						//initCombo<?=$objectId?>();
						url = base_url+'lke/kke2a_e2/save';  
						$("#kke2a_e2_id<?=$objectId?>").val(row.kke2a_e2_id);
				...
				//addTab("Add PK Kementerian", "lke/kke2a_e2/add");
			}
			...
			setTimeout(function(){
				/* $('#dg<?=$objectId;?>').datagrid({
				url:"<?=base_url()?>lke/kke2a_e2/grid",
				queryParams:{lastNo:'0'},		
					onLoadSuccess:function(data){
						$('#dg<?=$objectId;?>').datagrid('options').queryParams.lastNo = data.lastNo;
						prepareMerge<?=$objectId;?>(data);
	...
	<div id="tb<?=$objectId;?>" style="height:auto">
		...
		<td>
		  <div class="fsearch" >
			...
			<tr>
				<td>Tahun :</td>
				<td>
				<?=$this->sasaran_eselon2_model->getListFilterTahun($objectId,false)?>
				</td>				
			</tr>
			<tr>
	  ...
	  <? if($this->sys_menu_model->cekAkses('ADD;',318,$this->session->userdata('group_id'),$this->session->userdata('level_id'))){?>
			<a href="#" onclick="newData<?=$objectId;?>();" class="easyui-linkbutton" iconCls="icon-ok" plain="true">Set Kinerja</a>  
		<?}?>
	
		
		
			<? if($this->sys_menu_model->cekAkses('PRINT;',318,$this->session->userdata('group_id'),$this->session->userdata('level_id'))){?>
				<a 	href="#" onclick="printData<?=$objectId;?>();" class="easyui-linkbutton" iconCls="icon-print" plain="true">Print</a>
			<?}?>
			<? if($this->sys_menu_model->cekAkses('EXCEL;',318,$this->session->userdata('group_id'),$this->session->userdata('level_id'))){?>
				<a href="#" onclick="toExcel<?=$objectId;?>();" class="easyui-linkbutton" iconCls="icon-excel" plain="true">Excel</a>
			<?}?>
	  </div>
	</div>
	
	<table id="dg<?=$objectId;?>" style="height:auto;width:auto" title="Laporan KKE2A Sasaran Eselon II" toolbar="#tb<?=$objectId;?>" fitColumns="fakse" singleSelect="true" rownumbers="false" pagination="true"  nowrap="false" showFooter="true">
	  <thead>
	  <tr>
		
		<th field="no" rowspan="3" sortable="false"  halign="center" align="center"  width="25px">No.</th>
		<th field="kke2a_e2_id" rowspan="3" sortable="false" hidden="true" width="25px">kke2a_e2_id</th>
		...
		<th field="kode_sasaran_e2" rowspan="3" sortable="false" hidden="true" width="25px">kode_sasaran_e2</th>
	...
	<div id="dlg<?=$objectId;?>" class="easyui-dialog" style="width:720px;height:300px;padding:10px 20px" closed="true"  buttons="#dlg-buttons">
		...
		<form id="fm<?=$objectId;?>" method="post" >			
			...
			<div class="fitem">
				...
				<input type="hidden" id="kke2a_e2_id<?=$objectId?>" name="kke2a_e2_id"/>				
			</div>
			<div class="fitem">
