	<script  type="text/javascript" >
		$(function(){
			...
			getUrl<?=$objectId;?> = function (tipe){
				...
				if (tipe==1){
					return "<?=base_url()?>realisasi/rpt_akuntabilitaskl/grid/"+filtahun+"/"+filsasaran+"/"+filiku;
				}
				else if (tipe==2){
					return "<?=base_url()?>realisasi/rpt_akuntabilitaskl/pdf/"+filtahun+"/"+filsasaran+"/"+filiku+paging;
				}else if (tipe==3){
					return "<?=base_url()?>realisasi/rpt_akuntabilitaskl/excel/"+filtahun+"/"+filsasaran+"/"+filiku+paging;
				}
			...
			setTimeout(function(){
				searchData<?=$objectId;?>();
				/* $('#dg<?=$objectId;?>').datagrid({
					url:"<?=base_url()?>realisasi/rpt_akuntabilitaskl/grid",
					queryParams:{lastNo:'0'},	
					onLoadSuccess:function(data){
						$('#dg<?=$objectId;?>').datagrid('options').queryParams.lastNo = data.lastNo;
						//prepareMerge<?=$objectId;?>(data);
	  ...
	  <tr>
		<td>
		  <div class="fsearch" >
			...
			<tr>
				<td>Tahun :</td>
				<td><?=$this->rpt_akuntabilitaskl_model->getListTahun($objectId)?></td>
			</tr>			
		<!--	<tr>
				<td>Kode Sasaran :</td>
	   ...
	   <div style="margin-bottom:5px">  
			<? if($this->sys_menu_model->cekAkses('PRINT;',261,$this->session->userdata('group_id'),$this->session->userdata('level_id'))){?>
				<a 	href="#" onclick="printData<?=$objectId;?>();" class="easyui-linkbutton" iconCls="icon-print" plain="true">Print</a>
			<?}?>
			<? if($this->sys_menu_model->cekAkses('EXCEL;',261,$this->session->userdata('group_id'),$this->session->userdata('level_id'))){?>
