	<script  type="text/javascript" >
		$(function(){
			...
			//	if(filiku==null) 
				...
				if (tipe==1){
					return "<?=base_url()?>penetapan/rpt_penetapaneselon2/grid/"+filtahun+"/"+file2;
				}
				else if (tipe==2){
					return "<?=base_url()?>penetapan/rpt_penetapaneselon2/pdf/"+filtahun+"/"+file2+"/"+filsasaran+"/"+filiku+paging;
				}else if (tipe==3){
					return "<?=base_url()?>penetapan/rpt_penetapaneselon2/excel/"+filtahun+"/"+file2+"/"+filsasaran+"/"+filiku+paging;
				}
			...
			setTimeout(function(){
				/* $('#dg<?=$objectId;?>').datagrid({
					url:"<?=base_url()?>penetapan/rpt_penetapaneselon2/grid",
					queryParams:{lastNo:'0'},	
					onLoadSuccess:function(data){
						$('#dg<?=$objectId;?>').datagrid('options').queryParams.lastNo = data.lastNo;
						prepareMerge<?=$objectId;?>(data);
	  ...
	  <tr>
		<td>
		  <div class="fsearch" style="">
			...
			<tr>
				<td>Tahun :&nbsp</td>
				<td><?=$this->rpt_penetapaneselon2_model->getListTahun($objectId)?></td>
			</tr>
			<tr>
	   ...
	   <div style="margin-bottom:5px">  
			<? if($this->sys_menu_model->cekAkses('PRINT;',258,$this->session->userdata('group_id'),$this->session->userdata('level_id'))){?>
				<a 	href="#" onclick="printData<?=$objectId;?>();" class="easyui-linkbutton" iconCls="icon-print" plain="true">Print</a>
			<?}?>
			<? if($this->sys_menu_model->cekAkses('EXCEL;',258,$this->session->userdata('group_id'),$this->session->userdata('level_id'))){?>
				<a href="#" onclick="toExcel<?=$objectId;?>();" class="easyui-linkbutton" iconCls="icon-excel" plain="true">Excel</a>
			<?}?>
	  </div>
	</div>
	
	<table id="dg<?=$objectId;?>" class="easyui-datagrid" style="height:auto;width:auto" title="Laporan Penetapan Kinerja Unit Kerja Eselon II" toolbar="#tb<?=$objectId;?>" fitColumns="true" singleSelect="true" rownumbers="false" pagination="true"  nowrap="false" showFooter="true">
	  <thead>
	  <tr>
		...
		<th  sortable="false" colspan="2" width="150px">Indikator Kinerja Kegiatan</th>
