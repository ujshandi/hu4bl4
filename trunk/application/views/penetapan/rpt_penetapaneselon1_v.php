	<script  type="text/javascript" >
		$(function(){
			...
			clearFilter<?=$objectId;?> = function (){
				...
				//$('#dg<?=$objectId;?>').datagrid({url:"<?=base_url()?>penetapan/rpt_penetapaneselon1/grid/"+filtahun+"/"+filnama+"/"+filalamat});
					//$('#dg<?=$objectId;?>').datagrid({url:"<?=base_url()?>penetapan/rpt_penetapaneselon1/grid/"+filtahun+"/"+filnama+"/"+filalamat});
			}
			...
			getUrl<?=$objectId;?> = function (tipe){
				...
				if (tipe==1){
					return "<?=base_url()?>penetapan/rpt_penetapaneselon1/grid/"+filtahun+"/"+file1;
				}
				else if (tipe==2){
					return "<?=base_url()?>penetapan/rpt_penetapaneselon1/pdf/"+filtahun+"/"+file1+"/"+filsasaran+"/"+filiku+paging;
				}else if (tipe==3){
					return "<?=base_url()?>penetapan/rpt_penetapaneselon1/excel/"+filtahun+"/"+file1+"/"+filsasaran+"/"+filiku+paging;
				}
			...
			setTimeout(function(){
				/* $('#dg<?=$objectId;?>').datagrid({
					url:"<?=base_url()?>penetapan/rpt_penetapaneselon1/grid",
					queryParams:{lastNo:'0'},	
	...
	<div id="tb<?=$objectId;?>" style="height:auto">
		<table border="0" cellpadding="1" cellspacing="1" width="100%">
				...
				<td>
					<div class="fsearch" >
						<table border="0" cellpadding="1" cellspacing="1">
							<tr>
								<td>Tahun&nbsp</td>
								<td><?=$this->rpt_penetapaneselon1_model->getListTahun($objectId)?></td>
							</tr>
							<tr>
		 ...
		 <div style="margin-bottom:5px">  
			<? if($this->sys_menu_model->cekAkses('PRINT;',257,$this->session->userdata('group_id'),$this->session->userdata('level_id'))){?>
				<a 	href="#" onclick="printData<?=$objectId;?>();" class="easyui-linkbutton" iconCls="icon-print" plain="true">Print</a>
			<?}?>
			<? if($this->sys_menu_model->cekAkses('EXCEL;',257,$this->session->userdata('group_id'),$this->session->userdata('level_id'))){?>
				<a href="#" onclick="toExcel<?=$objectId;?>();" class="easyui-linkbutton" iconCls="icon-excel" plain="true">Excel</a>
			<?}?>
	  </div>
	</div>
	
	<table id="dg<?=$objectId;?>" class="easyui-datagrid" style="height:auto;width:auto" title="Laporan Penetapan Kinerja Unit Kerja Eselon I" toolbar="#tb<?=$objectId;?>" fitColumns="true" singleSelect="true" rownumbers="false" pagination="true" nowrap="false" showFooter="true">
	  <thead>
	  <tr>
		...
		<th  sortable="false" colspan="2" width="150px">Indikator Kinerja Utama</th>
		<th field="target"  align="right" rowspan="2" sortable="false" width="100px">Target</th>
