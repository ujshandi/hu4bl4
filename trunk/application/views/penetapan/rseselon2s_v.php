	<script  type="text/javascript" >
		$(function(){
			...
			clearFilter<?=$objectId;?> = function (){
				...
				//$('#dg<?=$objectId;?>').datagrid({url:"<?=base_url()?>realisasi/rseselon2/grid/"+filnip+"/"+filnama+"/"+filalamat});
			}
			...
			editData<?=$objectId;?> = function (){
				...
				if (row){
					$('#dlg<?=$objectId;?>').dialog('open').dialog('setTitle','Edit rseselon2');
					$('#fm<?=$objectId;?>').form('load',row);
					url = base_url+'realisasi/rseselon2/save';//+row.id;//'update_user.php?id='+row.id;
				}
			}
			//end editData
	...
	<table id="dg<?=$objectId;?>" class="easyui-datagrid" style="height:auto;width:auto" title="Capaian Kinerja Eselon II" toolbar="#tb<?=$objectId;?>" fitColumns="true" singleSelect="true" rownumbers="true" pagination="true">
	  ...
	  <tr>
		...
		<th field="kode_satker" sortable="true" width="50px">Kode Satker</th>
		<th field="kode_sasaran_e2" sortable="true" width="50px">Kode Sasaran Eselon II</th>
