	...
	<script  type="text/javascript" >
		...
		$(function(){
			...
			cancel<?=$objectId;?>=function(){
				$('#tt').tabs('close', '<?=($editmode==TRUE?'Edit':'View')?> Ongoing Eselon II');
			}
			
		 	saveDataEdit<?=$objectId;?>=function(){
				$('#fmedit<?=$objectId;?>').form('submit',{
					url: base_url+'kka/ongoing_e2/save_edit',
					onSubmit: function(){
					...
					success: function(result){
						...
						if (result.success){
							...
							$('#tt').tabs('close', 'Edit Ongoing Eselon II');
						} else {
							$.messager.show({
								title: 'Error',
								msg: result.msg
	...
	<div id="cc<?=$objectId;?>" class="easyui-layout" fit="true">  
		...
		<div region="north" split="true" title="" style="height:450px;">
			<div class="easyui-layout" fit="true">  
				...
				<div region="center" border="true" title="Edit Ongoing Eselon II">	
					<form id="fmedit<?=$objectId;?>" method="post" style="margin:10px 5px 5px 10px;">
						<input name="ongoinge2_id" class="" type="hidden" value="<?=$result->ongoinge2_id?>">
						<div class="fitem">
