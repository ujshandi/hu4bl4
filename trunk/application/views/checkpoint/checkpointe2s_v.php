	<script  type="text/javascript" >
		...
		$(function(){
			...
			newData<?=$objectId;?> = function (){  
				...
				if (row){
					$('#dlg<?=$objectId;?>').dialog('open').dialog('setTitle','Add <?=$purpose?> Checkpoint Eselon II');  
					$('#fm<?=$objectId;?>').form('clear');  
					url = base_url+'checkpoint/checkpointe2/save';  
					$("#deskripsi_iku_e2<?=$objectId?>").val(row.deskripsi_iku_e2);
					$("#deskripsi_sasaran_e2<?=$objectId?>").val(row.deskripsi_sasaran_e2);
					$("#id_pk_e2<?=$objectId?>").val(row.id_pk_e2);
					$("#penanggungjawab<?=$objectId?>").val(row.nama_e2);
					$("#kd_e2<?=$objectId?>").val(row.kode_e2);
				...
				//addTab("Add PK Eselon II", "checkpoint/checkpointe2/add");
			}
			//end newData 
			...
			//	$('#dg<?=$objectId;?>').datagrid('options').queryParams.
				...
				//if (row){
					$.ajax({
					url:'<?=base_url()?>checkpoint/checkpointe2/getDataEdit/'+idCheckpoint<?=$objectId;?>,
					success:function(data){
						//alert(data);
						var data = eval('('+data+')');
							$('#dlg<?=$objectId;?>').dialog('open').dialog('setTitle','Edit <?=$purpose?> Checkpoint Eselon II');  
						$('#fm<?=$objectId;?>').form('clear');  
						url = base_url+'checkpoint/checkpointe2/save';  
						$("#deskripsi_iku_e2<?=$objectId?>").val(data.deskripsi_iku_e2);
						$("#deskripsi_sasaran_e2<?=$objectId?>").val(data.deskripsi_sasaran_e2);
						$("#id_pk_e2<?=$objectId?>").val(data.id_pk_e2);
						$("#id_checkpoint_e2<?=$objectId?>").val(data.id_checkpoint_e2);
						$("#penanggungjawab<?=$objectId?>").val(data.nama_e2);
						...
						$("#nama_folder_pendukung<?=$objectId?>").val(row.nama_folder_pendukung);
						<? if ($purpose=='Capaian') {?>
						getListFilePendukung(data.kode_e2,data.id_pk_e2,data.periode);
						prepareUpload<?=$objectId?>();
						<?}?>
					}});	
			}
				...
				<? if ($this->session->userdata('unit_kerja_e2')=='-1'){?>				
					...
					success:function(data){
						...
						if(confirm("Apakah yakin akan menghapus data " + '' + "?")){
							var response = '';
							$.ajax({ type: "GET",
									 url: base_url+'checkpoint/checkpointe2/delete/' + data.id_checkpoint_e2,
									 async: false,
			...
			getUrl<?=$objectId;?> = function (tipe){
				...
				if (tipe==1){
					return "<?=base_url()?>checkpoint/checkpointe2/grid/"+filtahun+"/"+file2;
				}
				else if (tipe==2){
					return "<?=base_url()?>checkpoint/checkpointe2/pdf/"+filtahun+"/"+file2;
				}else if (tipe==3){
					return "<?=base_url()?>checkpoint/checkpointe2/excel/"+filtahun+"/"+file2;
				}
			...
			setTimeout(function(){
				//searchData<?=$objectId;?>();
				//$('#dg<?=$objectId;?>').datagrid({url:"<?=base_url()?>checkpoint/checkpointe2/grid"});
			},0);
			...
			getFolderName<?=$objectId?> = function(){
					var rs = base_url+"upload/pendukung/e2/";
					rs = rs + $("#kd_e2<?=$objectId?>").val()+'/'+$("#id_pk_e2<?=$objectId?>").val()+'/'+$("#cmbPeriode<?=$objectId?>").val();
			...
			deleteFilePendukung = function(kode_e2,id_pk_e2, periode,file){
				jQuery.ajax({
					url:base_url+'checkpoint/checkpointe2/deleteFile/'+kode_e2+'/'+id_pk_e2+'/'+periode+'/'+file,
					success: function(data, textStatus,jqXHR){
						$('#files<?=$objectId?>').empty();
						getListFilePendukung(kode_e2,id_pk_e2, periode);
					},
			...
			getListFilePendukung = function(kode_e2,id_pk_e2, periode){
				jQuery.ajax({
					url:base_url+'checkpoint/checkpointe2/getListFile/'+kode_e2+'/'+id_pk_e2+'/'+periode,
					success: function(data, textStatus,jqXHR){
		...
		<tr>
			...
			<div class="fsearch" <?=($this->session->userdata('unit_kerja_e2')=='-1'?'':'') //style="display:none"?>> 
				...
				<tr>
					<td>Tahun :</td>
					<td>
					<?=$this->checkpointe2_model->getListFilterTahun($objectId)?>
					</td>
				</tr>
					<tr>
					<td>Unit Kerja Eselon II&nbsp</td>
					<td>
						<?=$this->eselon1_model->getListFilterEselon1($objectId,$this->session->userdata('unit_kerja_e2'))?>
					</td>
				</tr>
	...
	<table id="dg<?=$objectId;?>" style="height:auto;width:auto"
	 title="Data Checkpoint Eselon II" toolbar="#tb<?=$objectId;?>" fitColumns="true" singleSelect="true" rownumbers="true" pagination="true">
	  ...
	  <tr>
		...
		<th field="deskripsi_iku_e2" hidden="true">deskripsi_iku_e2</th>
		<th field="deskripsi_sasaran_e2" hidden="true">deskripsi_sasaran_e2</th>
	  </tr>
	  </thead>  
	</table>
	...
	<div id="dlg<?=$objectId;?>" class="easyui-dialog" style="width:800px;height:450px;padding:10px 20px" closed="true" buttons="#dlg-buttons">
		<!----------------Edit title-->
		<div id="ftitle<?=$objectId?>" class="ftitle">Add/Edit/View Rencana Checkpoint Eselon II</div>
		<form id="fm<?=$objectId;?>" method="post">
			<input type="hidden" name="kd_e2<?=$objectId?>" id="kd_e2<?=$objectId?>" value=""/>
			<div class="fitem">
				<label style="width:130px">Sasaran Strategis:</label>					
					<textarea readonly name="deskripsi_sasaran_e2" id="deskripsi_sasaran_e2<?=$objectId?>" cols="70" class="easyui-validatebox" ></textarea>
				<input type="hidden" id="id_pk_e2<?=$objectId?>" name="id_pk_e2"/>
				<input type="hidden" id="id_checkpoint_e2<?=$objectId?>" name="id_checkpoint_e2"/>
			...
			<div class="fitem">
				...
				<textarea readonly name="deskripsi_iku_e2" id="deskripsi_iku_e2<?=$objectId?>" cols="70" class="easyui-validatebox" ></textarea>
			</div>
			<div class="fitem">
			...
			$('#dg<?=$objectId;?>').datagrid({
				...
				//	alert(row.id_pk_e2);
                    ...
                    $('#ddv<?=$objectId;?>-'+index).datagrid({
                        url:'<?=base_url()?>checkpoint/checkpointe2/griddetail/'+row.id_pk_e2+'/?parentIndex='+index,
                        fitColumns:true,
                        singleSelect:true,
                        rownumbers:true,
                        loadMsg:'',
                        height:'auto',
                        columns:[[
                            {field:'id_checkpoint_e2',title:'id',width:200,hidden:true},
                       ...
                       onClickCell:function(rowIndex, field, value){
							...
							idCheckpoint<?=$objectId;?> = row.id_checkpoint_e2;
...
/*global window, $ */
	...
	prepareUpload<?=$objectId?> = function(){
	      ...
	      url =  base_url+'checkpoint/checkpointe2/upload';

			jQuery('#fileupload<?=$objectId?>').unbind('fileupload').fileupload({
				url: base_url+'checkpoint/checkpointe2/upload/'+$("#kd_e2<?=$objectId?>").val()+'/'+$("#id_pk_e2<?=$objectId?>").val()+'/'+$("#cmbPeriode<?=$objectId?>").val(),
				dataType: 'json',
				...
				//singleFileUploads: false
					...
					$.each(data.result.files, function (index, file) {
						...
						//	var deleteUrl = base_url+"checkpoint_kl/upload/"+$("#kd_kl<?=$objectId?>").val()+'/'+$("#id_pk_kl<?=$objectId?>").val()+'/'+$("#cmbPeriode<?=$objectId?>").val();
							var deleteUrl = base_url+"checkpoint/chekcpointe2/upload/"+$("#kd_e2<?=$objectId?>").val()+'/'+$("#id_pk_e2<?=$objectId?>").val()+'/'+$("#cmbPeriode<?=$objectId?>").val()+"/?file="+file.name;
							row.find('.delete')
