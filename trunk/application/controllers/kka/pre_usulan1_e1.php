...
class Pre_usulan1_e1 extends CI_Controller {
	
	var $objectId = 'preusulan1e1';
	...
	function index(){
		...
		//$data['formLookupTarif'] = $this->tarif_model->lookup('#winLookTarif'.$data['objectId'],"#medrek_id".$data['objectId']);
	  	$this->load->view('kka/pre_usulan1_e1s_v',$data);
	}
	...
	function save_edit(){
		...
		if($data['target'] == '' || $data['target'] == null){
			...
			if($data['old_kode_iku_e1'] != $data['kode_iku_e1']){ // jika ada perubahan kode iku
				if(!$this->pre_usulan1_e1_model->data_exist($data['tahun'], $data['kode_e1'], $data['kode_sasaran_e1'], $data['kode_iku_e1'])){
					$result = $this->pre_usulan1_e1_model->UpdateOnDb($data);
				}else{
					$result = false;
	...
	function delete($id=''){
		if($id != ''){
			$result = $this->pre_usulan1_e1_model->DeleteOnDb($id);
			if ($result){
	...
	function getSatuan($id){
		if($id != '0'){
			echo $this->pre_usulan1_e1_model->getSatuan($id);
		}
	}
	...
	public function pdf($file1=null){
		...
		$pdfdata = $this->pre_usulan1_e1_model->easyGrid($file1,2);
	...
	public function excel($file1=null){
		echo  $this->pre_usulan1_e1_model->easyGrid($file1,3);
	}
}
?>