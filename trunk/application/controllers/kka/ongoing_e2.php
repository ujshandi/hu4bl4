...
class Ongoing_e2 extends CI_Controller {
	
	var $objectId = 'ongoinge2';
	...
	function __construct()
	{
		...
		$this->load->model('/kka/ongoing_e2_model');
	...
	function index(){
		$data['title'] = 'Data Ongoing Eselon II';	
		$data['objectId'] = $this->objectId;
		//$data['formLookupTarif'] = $this->tarif_model->lookup('#winLookTarif'.$data['objectId'],"#medrek_id".$data['objectId']);
	  	$this->load->view('kka/ongoing_e2s_v',$data);
	}
	
	public function add(){
		$data['title'] = 'Add Data Ongoing Eselon II';	
		$data['objectId'] = $this->objectId;
		//$data['formLookupTarif'] = $this->tarif_model->lookup('#winLookTarif'.$data['objectId'],"#medrek_id".$data['objectId']);
	  	$this->load->view('kka/ongoing_e2_v',$data);
	}
	
	public function edit($id, $editmode='true'){
		$editmode = ($editmode=='true'?TRUE:FALSE);	
		$data['editmode'] = $editmode;
		$data['title'] = ($editmode==TRUE?'Edit':'View').' Data Ongoing Eselon II';
		$data['objectId'] = $this->objectId;
		
		$data['result'] = $this->ongoing_e2_model->getDataEdit($id);
		
	  	$this->load->view('kka/ongoing_e2_edit_v',$data);
	}
	...
	function grid($filtahun=null,$file1=null){
		...
		echo $this->ongoing_e2_model->easyGrid($filtahun,$file1);
	}
	...
	function getpendukung($filtahun=null,$filtriwulan,$file2=null,$filsasaran=null, $filikk=null){
		...
		echo $this->ongoing_e2_model->getPendukung($filtahun,$filtriwulan,$file2,$filsasaran,$filikk);
	}
		...
		if ($this->form_validation->run() == FALSE){ // jika tidak valid
			...
			if ($data['kode_sasaran_e2']=='0') {
				...
				if($this->check_detail($data, $pesan)){
					$result = $this->ongoing_e2_model->InsertOnDb($data);
				}else{
	...
	function check_detail($data, & $pesan){
		...
		/* 	// cek ke database
			if($this->ongoing_e2_model->data_exist($data['tahun'], $data['kode_e1'], $data['kode_sasaran_e1'], $r['kode_ikk'])){ 
	...
	function save_edit(){
		...
		$data['ongoinge2_id'] 			= $this->input->post('ongoinge2_id');
		...
		if($data['jumlah'] == '' || $data['jumlah'] == null){
			...
			$result = $this->ongoing_e2_model->UpdateOnDb($data);			
		}
		
		if ($result){
			echo json_encode(array('success'=>true, 'jumlah'=>$data['jumlah']));
		} else {
	...
	function delete($id=''){
		if($id != ''){
			$result = $this->ongoing_e2_model->DeleteOnDb($id);
			if ($result){
	...
	function getSatuan($id){
		if($id != '0'){
			echo $this->ongoing_e2_model->getSatuan($id);
		}
	}
	
	function getikk($kode, $tahun=""){
		if($kode != '0' && $tahun != ""){
			echo $this->ongoing_e2_model->getikk($this->objectId, $kode, $tahun);
		}
	}
	
	function getKegiatan_e2($kode, $tahun="",$kodesasaran="",$kodeikk=""){
		if($kode != '0' && $tahun != ""){
			echo $this->ongoing_e2_model->getKegiatan_e2($this->objectId, $kode, $tahun,true,$kodesasaran,$kodeikk);
		}
	}
	...
	public function pdf($file1=null){
		...
		$pdfdata = $this->ongoing_e2_model->easyGrid($file1,2);
	...
	public function excel($file1=null){
		echo  $this->ongoing_e2_model->easyGrid($file1,3);
	}
}
?>
