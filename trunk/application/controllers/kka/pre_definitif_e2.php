...
class Pre_definitif_e2 extends CI_Controller {
	...
	public function edit($id, $editmode='true'){
		...
		$data['title'] = ($editmode==TRUE?'Edit':'View').' Data Pra Monev Eselon II Pagu Definitif';
		$data['objectId'] = $this->objectId;
		
		$data['result'] = $this->pre_definitif_e2_model->getDataEdit($id);
		
	  	$this->load->view('kka/pre_definitif_e2_edit_v',$data);
	}
	...
	function save_edit(){
		...
		$data['predefinitif_e2_id'] 			= $this->input->post('predefinitif_e2_id');
	...
	function getKegiatan_e2($kode, $tahun="",$kodesasaran="",$kodeikk=""){
		if($kode != '0' && $tahun != ""){
			echo $this->pre_definitif_e2_model->getKegiatan_e2($this->objectId, $kode, $tahun,true,$kodesasaran,$kodeikk);
		}
	}
