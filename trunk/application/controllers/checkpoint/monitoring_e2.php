...
class Monitoring_e2 extends CI_Controller {
	...
	//if ($this->session->userdata('logged_in') != TRUE) redirect('security/login');					
		...
		$this->load->model('/checkpoint/monitoring_e2_model');
	...
	function index()
	{
		...
		$this->load->view('checkpoint/monitoring_e2_vw',$data);
	...
	public function grid($filtahun=null,$filperiode=null){
		...
		echo $this->monitoring_e2_model->easyGrid($filtahun,$filperiode);
	}
	
	
	function getDataPie($filtahun=null){
		   $data = $this->monitoring_e2_model->dataPie;//array("Tercapai"=>20,"Tidak Tercapai"=>3);
