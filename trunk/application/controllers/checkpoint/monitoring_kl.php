...
class Monitoring_kl extends CI_Controller {
	var $dataPie = array();
	...
	//if ($this->session->userdata('logged_in') != TRUE) redirect('security/login');					
		...
		$this->load->model('/checkpoint/monitoring_kl_model');
	...
	function index()
	{
		...
		$this->load->view('checkpoint/monitoring_kl_vw',$data);
	...
	public function grid($filtahun=null,$filperiode=null){
		...
		echo $this->monitoring_kl_model->easyGrid($filtahun,$filperiode);
		 $this->dataPie = $this->monitoring_kl_model->dataPie;
	...
	function getDataPie($filtahun=null){
		//  $data = $this->monitoring_kl_model->dataPie;// 
		  
		// $this->monitoring_kl_model->easyGrid($filtahun);
		// $this->dataPie = $this->monitoring_kl_model->dataPie;
		//  $data = array("Tercapai"=>20,"Tidak Tercapai"=>3);
		//	var_dump($this->dataPie);die;
